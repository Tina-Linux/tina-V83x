...
int main(int argc, char *argv[])
{
    ...
    //pContext->mpRecorder->setCamera(pContext->mpCamera);
    ...
    pContext->mMuxerId = pContext->mpRecorder->addOutputFormatAndOutputSink(MEDIA_FILE_FORMAT_MP4, (char*)strSegmentFilePath.c_str(), 0, false);
    pContext->mSegmentFiles.push_back(strSegmentFilePath);
    alogd("setVideoFrameRate=[%d]", pContext->mConfigPara.mFrameRate);
    pContext->mpRecorder->setVideoFrameRate(pContext->mConfigPara.mFrameRate);
    alogd("setEncodeVideoSize=[%dx%d]", pContext->mConfigPara.mEncodeWidth, pContext->mConfigPara.mEncodeHeight);
    pContext->mpRecorder->setVideoSize(pContext->mConfigPara.mEncodeWidth, pContext->mConfigPara.mEncodeHeight);
    pContext->mpRecorder->setVideoEncoder(pContext->mConfigPara.mEncodeType);
    pContext->mpRecorder->setVideoEncodingRateControlMode(EyeseeRecorder::VideoRCMode_CBR);
    ...
    //pContext->mpRecorder->setVideoEncodingBitRate(pContext->mConfigPara.mEncodeBitrate);
    alogd("setSourceChannel=[%d]", cameraInfo.mMPPGeometry.mISPGeometrys[0].mScalerOutChns[0]);
    //pContext->mpRecorder->setSourceChannel(cameraInfo.mMPPGeometry.mScalerOutChns[0]);
