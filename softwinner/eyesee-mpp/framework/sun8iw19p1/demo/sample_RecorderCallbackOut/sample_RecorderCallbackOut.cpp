...
#include "sample_RecorderCallbackOut_config.h"
#include "sample_RecorderCallbackOut.h"

using namespace std;
using namespace EyeseeLinux;

SampleRecorderCallbackOutContext *pSampleRecorderCallbackOutContext = NULL;

void handle_exit(int signo)
{
    alogd("user want to exit!");
    if(pSampleRecorderCallbackOutContext!=NULL)
    {
        cdx_sem_up(&pSampleRecorderCallbackOutContext->mSemExit);
    }
}
...
EyeseeRecorderListener::EyeseeRecorderListener(SampleRecorderCallbackOutContext *pOwner)
    : mpOwner(pOwner)
{
}
SampleRecorderCallbackOutContext::SampleRecorderCallbackOutContext()
...
status_t SampleRecorderCallbackOutContext::ParseCmdLine(int argc, char *argv[])
{
    ...
    while(i < argc)
    {
        ...
        else if(!strcmp(argv[i], "-h"))
        {
            ...
            helpString += "\t run -path /home/sample_RecorderCallbackOut.conf\n";
...
status_t SampleRecorderCallbackOutContext::loadConfig()
{
    int ret;
    char *ptr;
    ...
    if(ConfigFilePath.empty())
    {
        ...
        mConfigPara.mEncodeFolderPath = "/home/sample_RecorderCallbackOut_Files";
        mConfigPara.mEncodeWidth = 1920;
        mConfigPara.mEncodeHeight = 1080;
        mConfigPara.mEncodeBitrate = 6*1024*1024;
        mConfigPara.mFilePath = "CallbackOut.raw";
    ...
    mConfigPara.mCaptureWidth = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_CAPTURE_WIDTH, 0); 
    mConfigPara.mCaptureHeight = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_CAPTURE_HEIGHT, 0); 
    char *pStrPixelFormat = (char*)GetConfParaString(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_CAPTURE_FORMAT, NULL); 
    ...
    mConfigPara.mPreviewWidth = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_PREVIEW_WIDTH, 0); 
    mConfigPara.mPreviewHeight = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_PREVIEW_HEIGHT, 0); 
    pStrPixelFormat = (char*)GetConfParaString(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_PREVIEW_FORMAT, NULL); 
    if(!strcmp(pStrPixelFormat, "yu12"))
    {
    ...
    mConfigPara.mbPreviewEnable = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_PREVIEW_ENABLE, 0);
    mConfigPara.mbEncodeEnable = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_ENCODE_ENABLE, 0);
    mConfigPara.mFrameRate = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_FRAME_RATE, 0); 
    mConfigPara.mDisplayFrameRate = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_DISPLAY_FRAME_RATE, 0);
    mConfigPara.mCaptureBufNum = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_CAPTURE_BUF_NUM, 0); 
    mConfigPara.mPreviewBufNum = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_PREVIEW_BUF_NUM, 0); 
    char *pStrEncodeType = (char*)GetConfParaString(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_ENCODE_TYPE, NULL); 
    ...
    mConfigPara.mEncodeDuration = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_ENCODE_DURATION, 0);
    mConfigPara.mEncodeFolderPath = GetConfParaString(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_ENCODE_FOLDER, NULL);

    mConfigPara.mEncodeWidth = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_ENCODE_WIDTH, 0);
    mConfigPara.mEncodeHeight = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_ENCODE_HEIGHT, 0);
    mConfigPara.mEncodeBitrate = GetConfParaInt(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_ENCODE_BITRATE, 0)*1024*1024;
    mConfigPara.mFilePath = GetConfParaString(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_FILE_PATH, NULL);
    mConfigPara.mFilePath2 = GetConfParaString(&stConfParser, SAMPLE_RECORDERCALLBACKOUT_KEY_FILE_PATH2, NULL);
    alogd("path[%s], path2[%s]", mConfigPara.mFilePath.c_str(), mConfigPara.mFilePath2.c_str());
    destroyConfParser(&stConfParser);
    return SUCCESS;
}

status_t SampleRecorderCallbackOutContext::CreateFolder(const std::string& strFolderPath)
{
...
{
    int result = 0;
    cout<<"hello, sample_RecorderCallbackOut!"<<endl;
    SampleRecorderCallbackOutContext stContext;
    pSampleRecorderCallbackOutContext = &stContext;
    ...
    if(stContext.mConfigPara.mbEncodeEnable)
    {
        ...
        if(!stContext.mConfigPara.mFilePath2.empty())
        {
            ...
            stContext.mpRecorder->addOutputFormatAndOutputSink(MEDIA_FILE_FORMAT_MP4, (char*)strFilePath2.c_str(), 0, false);
        }
        if(!stContext.mConfigPara.mFilePath.empty())
        {
            std::string strRawFilePath = stContext.mConfigPara.mEncodeFolderPath + '/' + stContext.mConfigPara.mFilePath;
    ...
    cout<<"bye, sample_RecorderCallbackOut!"<<endl;
    return result;
}
