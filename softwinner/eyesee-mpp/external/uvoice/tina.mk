# Makefile for eyesee-mpp/external/uvoice
CUR_PATH := .
PACKAGE_TOP := ..
...
ifeq ($(MPPEXTERNALCFG_UVOICE), Y)
	mkdir -p $(CUR_PATH)/out/usr/lib
	mkdir -p $(CUR_PATH)/out/usr/bin
	-cp -fpR $(CUR_PATH)/libuvoice.a $(CUR_PATH)/out/usr/lib
	-cp -fpR $(CUR_PATH)/libuv_se_bf.a $(CUR_PATH)/out/usr/lib
	-cp -fpR $(CUR_PATH)/libuvoice_wakeup.so $(CUR_PATH)/out/usr/lib
	-cp -fpR $(CUR_PATH)/libremove_click.a $(CUR_PATH)/out/usr/lib
	-cp -fpR $(CUR_PATH)/kws.bin $(CUR_PATH)/out/usr/bin
	@echo ===================================
	@echo build eyesee-mpp-external-uvoice done
	@echo ===================================
else
	@echo not build eyesee-mpp-external-uvoice
endif
