...
//        aloge("alsa_elem:%s",elem_name);

        if ( !strcmp(elem_name, AUDIO_AD0L_MIXER) ||
        ...
        else if (!strcmp(elem_name, AUDIO_LINEOUTL_MUX)) 
        {
            // for LINEOUTL, 0: LOMIX -> LINEOUTL Mux; 1: OMIX_LR_SUM ->  LINEOUTL Mux
            snd_mixer_selem_set_enum_item(elem, 0, 0);    // increase play volume when amplifier differential input
        } 
...
int alsaMixerSetVolume(AIO_MIXER_S *mixer, int playFlag, long value)
{
    ...
    if (playFlag && ((value < 0) || (value > 100))) {
        aloge("want to setAIVol[0,100], but usr value=%ld is invalid!", value);
        return -1;
    }

    if (!playFlag && ((value < 0) || (value > 31))) {
        aloge("want to setAIVol[0,31], but usr value=%ld is invalid!", value);
        return -1;
    }
    ...
    for (elem = snd_mixer_first_elem(mixer->handle); elem; elem = snd_mixer_elem_next(elem)) {
        ...
        else if(!playFlag && !strcmp(elem_name, AUDIO_MIC1_MAIN_GAIN))
        {
            ...
            alogd("set_ai_main_gain:%d",value);
...
int alsaMixerGetVolume(AIO_MIXER_S *mixer, int playFlag, long *value)
{
    ...
    for (elem = snd_mixer_first_elem(mixer->handle); elem; elem = snd_mixer_elem_next(elem)) {
        ...
        else if(!playFlag && !strcmp(elem_name, AUDIO_MIC1_MAIN_GAIN))
        {
            ...
            *value = realVal;
            alogd("get_ai_main_gain:%d",realVal);
        }
    }
    return err;
}
