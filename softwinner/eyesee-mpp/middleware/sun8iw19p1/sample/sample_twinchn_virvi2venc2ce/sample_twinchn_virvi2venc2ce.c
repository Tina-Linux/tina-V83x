...
static ERRORTYPE InitVi2Venc2CeData(SAMPLE_VI2VENC2CE_S *pVi2Venc2CeData)
{
    ...
    pVi2Venc2CeData->mConfigPara.mField = VIDEO_FIELD_FRAME;
...
{
    SAMPLE_VI2VENC2CE_S *venc = (SAMPLE_VI2VENC2CE_S *)cookie;
...
static ERRORTYPE configVencChnAttr(SAMPLE_VI2VENC2CE_S *pVi2Venc2CeData)
{
    ...
    pVi2Venc2CeData->mVencChnAttr.VeAttr.Field = pVi2Venc2CeData->mConfigPara.mField;
    ...
    if (PT_H264 == pVi2Venc2CeData->mVencChnAttr.VeAttr.Type)
    {
        ...
        case 1:
            ...
            pVi2Venc2CeData->mVencChnAttr.RcAttr.mAttrH264Vbr.mMinQp = 10;
            pVi2Venc2CeData->mVencChnAttr.RcAttr.mAttrH264Vbr.mMaxQp = 52;
            break;
        case 2:
    ...
    else if (PT_H265 == pVi2Venc2CeData->mVencChnAttr.VeAttr.Type)
    {
        ...
        case 1:
            pVi2Venc2CeData->mVencChnAttr.RcAttr.mRcMode = VENC_RC_MODE_H264VBR;
            pVi2Venc2CeData->mVencChnAttr.RcAttr.mAttrH265Vbr.mMinQp = 10;
            pVi2Venc2CeData->mVencChnAttr.RcAttr.mAttrH265Vbr.mMaxQp = 52;
            break;
        case 2:
            pVi2Venc2CeData->mVencChnAttr.RcAttr.mRcMode = VENC_RC_MODE_H264FIXQP;
...
//                aloge("decout:%s,dec_len:%d",decout,dec_len);
                ...
                if (encout != NULL && decout != NULL) {
                    ...
                    if(decout_len == venc_frame_len){
                        aloge("openssl enc and dec test succese %d!",nViDev);
                    }
                    free(encout);
                    free(decout);
                }
...
static ERRORTYPE stop(SAMPLE_VI2VENC2CE_S *pVi2Venc2CeData)
{
    ...
    if (pVi2Venc2CeData->mViChn >= 0)
    {
        AW_MPI_VI_DestoryVirChn(pVi2Venc2CeData->mViDev, pVi2Venc2CeData->mViChn);
        AW_MPI_VI_DisableVipp(pVi2Venc2CeData->mViDev);
        AW_MPI_VI_DestoryVipp(pVi2Venc2CeData->mViDev);
...
err_out_0:
	...
	if (result == 0) {
		printf("sample_twin_virvi2venc2ce exit!\n");
	}
