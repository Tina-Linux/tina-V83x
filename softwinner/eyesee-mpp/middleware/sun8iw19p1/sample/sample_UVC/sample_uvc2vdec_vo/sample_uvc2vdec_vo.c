...
int main(int argc, char **argv)
{
    ...
    if(pContext->mConfigParam.mbDecodeSubOutEnable)
    {
        ...
        {
            eRet = AW_MPI_VO_EnableChn(pContext->mSubVoLayer, pContext->mSubVOChn);
            if(SUCCESS == eRet)
            {
                bSuccessFlag = TRUE;
                break;
    ...
    if(pContext->mConfigParam.mbDecodeSubOutEnable)
    {
        AW_MPI_VO_DisableChn(pContext->mSubVoLayer, pContext->mSubVOChn);
        pContext->mSubVOChn = MM_INVALID_CHN;
    }
...
_exit:
    ...
    free(pContext);
    printf("sample_uvc2vdec_vo exit[%d]!\n", result);
    return result;
