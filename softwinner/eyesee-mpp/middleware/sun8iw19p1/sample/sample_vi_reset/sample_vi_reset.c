...
int DestroyOneVipp(VI_DEV nVippIndex, SampleViResetContext *pContext)
{
    ...
    ret = AW_MPI_VI_DestoryVipp(nVippIndex);
    if(ret != SUCCESS)
    {
...
int main(int argc, char *argv[])
{
    ...
    /* parse config file. */
    if(LoadSampleViResetConfig(&pContext->mConfigPara) != SUCCESS)
    {
    ...
    while (pContext->mTestNum < pContext->mConfigPara.mTestCount || 0 == pContext->mConfigPara.mTestCount)
    {
        ...
        {
            RunOneVipp(nVippIndex, pContext);
        }
        ...
        for(nVippIndex = pContext->mConfigPara.mVippStart; nVippIndex <= pContext->mConfigPara.mVippEnd; nVippIndex++)
        {
            for (virvi_chn = 0; virvi_chn < 1; virvi_chn++)
            {
                pthread_join(pContext->mVirViChnArray[nVippIndex][virvi_chn].mThid, NULL);
                alogd("vipp[%d]virChn[%d] capture thread is exit!", nVippIndex, virvi_chn);
            }
        }
        for(nVippIndex = pContext->mConfigPara.mVippStart; nVippIndex <= pContext->mConfigPara.mVippEnd; nVippIndex++)
        {
            DestroyOneVipp(nVippIndex, pContext);
        }
