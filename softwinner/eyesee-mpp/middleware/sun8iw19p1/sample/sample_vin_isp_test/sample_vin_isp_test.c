...
static int disp_init(int width, int height)
{
	...
	//1.close 图层channel 0  防止重新开启时开始几帧有绿色图
...
static int disp_set_addr(u32 addr0, u32 addr1, u32 addr2)
{
...
static void *loop_cap(void *pArg)
{
	...
	while (i < save_cnt) {
		...
		if (save_cnt < 10000) {
			...
			disp_set_addr(privCap->buffer.planes[0].mem_phy, privCap->buffer.planes[1].mem_phy, privCap->buffer.planes[2].mem_phy);
			i = i % save_cnt;
		}
...
int main_test(int ch_num, int width, int height, int out_fmt)
{
	...
	for(i = 0; i < ch_num; i++) {
		...
		case 3: privCap[i].vfmt.format.pixelformat = V4L2_PIX_FMT_NV21M; break;
