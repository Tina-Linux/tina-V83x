...
#ifdef MINIGUI_VERSION
int extract7Zip(char *fileName, unsigned char *fileBuffer, unsigned long *fileSize);
#if 0
int main(int argc, char *argv[])
{
	char buffer[1024*1024*4];
	int fileSize;
	extract7Zip(argv[1], buffer, &fileSize);
	int fd = open("tmp.bin", O_RDWR|O_CREAT, 0777);
	if (fd < 0) {
		fprintf(stderr, "fail to open tmp.bin %s\n", strerror(errno));
		return -1;
	}
...
#else
			memcpy(fileBuffer + fileOff, outBuffer + offset, processedSize);
			fileOff += processedSize;
#endif
          #ifdef USE_WINDOWS_FILE
          if (f->AttribDefined)
