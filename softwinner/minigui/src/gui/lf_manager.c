...
BOOL mg_InitLFManager (void)
{
    ...
    if (rdr->init && 0 != rdr->init (rdr)) {
        fprintf (stderr,"GUI>InitLFM: failed to initialize default renderer's private info.\n");
        return FALSE;
    }
    ...
    for (i = 1; i < MAX_NR_RENDERERS; ++i) 
    {
        ...
        if (rdr->init && 0 != rdr->init (rdr)) {
            wnd_lf_info[i].wnd_rdr = NULL;
            fprintf (stderr,"GUI>InitLFM: fail to initialize renderer[%d]'s private info.\n", i);
            continue;
        }
    }
