...
#define svVScroll_listview(scrollval, newpos, listviewflag) \
            svScroll_listview (hWnd, pscrdata, scrollval, newpos, FALSE, listviewflag);

BOOL top_line_flag = FALSE;
BOOL bottom_line_flag = FALSE;
/* ---------------------------------------------------------------------------- */
...
static void svScroll_listview (HWND hWnd, PSCRDATA pscrdata, int scrollval, int newpos, BOOL bHScroll, BOOL listviewflag)
{
    int scrollBoundMax;
    int scrollBoundMin;
    int scroll = 0;
    ...
    //printf("####nOffset%d,scrollval%d,newpos%d####\\n",nOffset,scrollval,newpos);
    scroll = (scrollval>0?scrollval:-scrollval);
    ...
    if (!bHScroll && listviewflag){
        if(nOffset == scrollBoundMin || pscrdata->nContY == scrollBoundMin){
            if(bottom_line_flag){
            ...
            rcDraw.left = 0;
            rcDraw.top = 0;
            rcDraw.right = pscrdata->visibleWidth;
            rcDraw.bottom = pscrdata->visibleHeight;
            SetPenWidth(hdc,6);
            ...
            top_line_flag = TRUE;
            ReleaseDC (hdc);
        }else if(nOffset == scrollBoundMax || pscrdata->nContY == scrollBoundMax){
            if(top_line_flag){
                UpdateWindow(hWnd,TRUE);
                top_line_flag = FALSE;
            }
        ...
        //printf("###scrollBoundMax%d,nOffset%d,pscrdata->nContX%d,pscrdata->nContY%d,scrollBoundMax%d,pscrdata->visibleWidth%d,pscrdata->visibleHeight%d,pscrdata->nContHeight%d,pscrdata->nContWidth%d\n",\
          //scrollBoundMax,nOffset,pscrdata->nContX,pscrdata->nContY,scrollBoundMax,pscrdata->visibleWidth,pscrdata->visibleHeight,pscrdata->nContHeight,pscrdata->nContWidth);
...
void scrolled_vscroll (HWND hWnd, PSCRDATA pscrdata, WPARAM wParam, LPARAM lParam)
{
    ...
    else if (wParam == SB_THUMBTRACK_LISTVIEW) {
        newpos = lParam;
        BOOL listview_flag = TRUE;
        svVScroll_listview (vscroll, newpos, listview_flag);
        return;
    }
    else if (wParam == SB_THUMBPOSITION) {
        newpos = lParam;
