...
/**
 * A class that makes calls to the underlying PryonLite wake word engine and
 * notifies observers of keyword events being triggered.
 */
...
public:
     ...
     *
     * @param stream The stream of audio data. This audio stream should be PCM
     ...
     * @param pathToModelFile The path to the desired model file to use for
     * detections. This parameter is used if the macro
     * KWD_AMAZONLITE_DYNAMIC_MODEL_LOADING is defined.
     * @param detectThreshold The threshold to use for detections. This must be a
     * value from 1-1000. A value of 1 will result in the most detections and a
     * value of 1000 will result in the fewest detections.
     * @param voiceActivityDetectionCallback An optional callback to be notified
     * of voice activity detection state changes. The definition of the required
     * function signature may be found in pryon_lite.h.
     * @return A new @c PryonLiteKeywordDetector, or @c nullptr if the operation
     * failed.
     */
    ...
    /**
     * Destructor. This joins on the internal detection loop thread and resets the
     * PryonLite engine.
     */
...
private:
    /*
     * The callback that PryonLite will use to notify this class of wake word
     * detections.
     *
     * @param handle A handle to the PryonLite decoder instance.
     * @param result PryonLite result containing the indices in which a detection
     * occurred and PryonLite metadata to be sent to AVS.
     * @note The maximum size for the PryonLite metadata is 2K, otherwise it won't
     * be sent.
     */
    ...
    /**
     * Initializes the PryonLite engine and creates a new decoder that that the
     * client will be primarily interacting with. This function should only be
     * called once with each new @c PryonWakeWordDetector.
     *
     * @param pathToModelFile The path to the desired model file to use for
     * detections. This parameter is used if the macro
     * KWD_AMAZONLITE_DYNAMIC_MODEL_LOADING is defined.
     * @param detectThreshold Integer from 1-1000. Default is 500. The higher the
     * threshold, the fewer detections.
     * @return @c true if a new decoder was created successfully and @c false if
     * there were any errors with initialization.
     */
    bool init(
        const std::string& pathToModelFile,
    ...
    /// PryonLite maintains the internal indices that do not necessarily coincide with the ones of the writer's stream.
    /// So this variable serves as an offset reference point used when notifying observers of keyword detection indices.
    avsCommon::avs::AudioInputStream::Index m_beginIndexOfStreamReader;

    /// Variable to store the sum of all jumps that may have occurred as the result of buffer overruns.
    avsCommon::avs::AudioInputStream::Index m_bufferOverrunJumpCounter;
    ...
    /// Internal thread that reads audio from the buffer and feeds it to
    /// PryonLite.
