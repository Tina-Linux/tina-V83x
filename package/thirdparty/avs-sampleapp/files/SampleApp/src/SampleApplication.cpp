...
#ifdef TCONFIGS
    std::string tconfigsJsonFile = pathToInputFolder.empty() ?
        "/etc/avs/tconfigs.json" : pathToInputFolder + "/tconfigs.json";
    auto tconfigsPlatformAdapter = TconfigsPlatformAdapter::create(tconfigsJsonFile);
    if (!tconfigsPlatformAdapter) {
        ACSDK_CRITICAL(LX("Failed to create TconfigsPlatformAdapter!"));
        return false;
    }
#endif
...
#elif defined(TCONFIGS)
    std::shared_ptr<TconfigsMicrophoneWrapper> micWrapper = TconfigsMicrophoneWrapper::create(
            sharedDataStream, tconfigsPlatformAdapter);
#else
#error "No audio input provided"
#endif
    if (!micWrapper) {
        ACSDK_CRITICAL(LX("Failed to create MicrophoneWrapper!"));
        return false;
    }
...
#ifdef TCONFIGS
    m_buttonInputManager = TconfigsButtonInputManager::create(
            m_interactionManager, tconfigsPlatformAdapter);
    if (!m_buttonInputManager) {
