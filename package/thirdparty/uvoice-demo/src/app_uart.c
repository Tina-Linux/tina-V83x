...
int app_uart_open(int fd,const char* port)
{
    ...
    if (fcntl(fd, F_SETFL, 0) < 0) {
        ...
        LOGI("fcntl=%d",fcntl(fd, F_SETFL,0));
    }
    /*Test whether terminal device*/
    if (0 == isatty(STDIN_FILENO)) {
        LOGW("standard input is not a terminal device");
        /*return(FALSE);*/
    } else {
        LOGI("isatty success!");
    }
