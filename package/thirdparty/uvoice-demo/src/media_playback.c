...
int ctx_init(struct ctx* ctx, const struct cmd *cmd)
{
    ...
    if (cmd->filename == NULL) {
        LOGW("filename not specified");
        return -1;
    }

    ctx->file = fopen(cmd->filename, "rb");
    if (ctx->file == NULL) {
        LOGE("failed to open '%s'", cmd->filename);
        return -1;
    }

    LOGD("ctx_init:  cmd->filetype = %p ,cmd->filetype = %s", cmd->filetype, cmd->filetype);
    if ((cmd->filetype != NULL) && (strcmp(cmd->filetype, "wav") == 0)) {
        if (fread(&ctx->wave_header, sizeof(ctx->wave_header), 1, ctx->file) != 1){
            LOGE("error: '%s' does not contain a riff/wave header", cmd->filename);
            fclose(ctx->file);
            return -1;
        }
        if ((ctx->wave_header.riff_id != ID_RIFF) ||
            (ctx->wave_header.wave_id != ID_WAVE)) {
            LOGE("error: '%s' is not a riff/wave file", cmd->filename);
            fclose(ctx->file);
            return -1;
        }
        ...
        do {
            if (fread(&ctx->chunk_header, sizeof(ctx->chunk_header), 1, ctx->file) != 1){
                LOGE("error: '%s' does not contain a data chunk", cmd->filename);
                fclose(ctx->file);
                return -1;
            }
            ...
            case ID_FMT:
                if (fread(&ctx->chunk_fmt, sizeof(ctx->chunk_fmt), 1, ctx->file) != 1){
                    LOGE("error: '%s' has incomplete format chunk", cmd->filename);
                    fclose(ctx->file);
                    return -1;
                }
    ...
    if (ctx->pcm == NULL) {
        ...
        LOGE("failed to open for pcm %u,%u", cmd->card, cmd->device);
        fclose(ctx->file);
        pcm_close(ctx->pcm);
        return -1;
    }
...
void playback(char *path, struct data_config *handle_config)
{
    ...
    if(ctx.savePcmFp == NULL){
        LOGW("open %s fail",save_pcm_path);
    }
#endif
    if (play_adpcm(&ctx) < 0) {
        ctx_free(&ctx);
        LOGE("play_adpcm error");
    }
    ...
    if(ctx.savePcmFp){
        ...
        ctx.savePcmFp = NULL;
            ...
            LOGW("err:it is neither pcm nor adpcm, can't play");
        }
    ctx_free(&ctx);
