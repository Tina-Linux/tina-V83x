...
static void check_filesize(int *fd)
{
	...
	//printf("filesize %d\n", stat_.st_size);
	if (stat_.st_size > 1024 * 1024 * filesize)
	{
...
//		snprintf(to_name, 128, "%s/%s_%s.cfa", logpath, basename, buff);
		...
		//printf("filename length before appending is %d\n", g_slist_length(filenames));
		n = g_slist_length(filenames);
		if ( n < max_num_of_file) {
			//printf("appending\n");
			filenames = g_slist_append(filenames, (gpointer)strdup(to_name));
		} else if (n == max_num_of_file) {
		//	printf("remove and appending\n");
			unlink(g_slist_nth_data(filenames, 0));
		//	printf("filename length before remove is %d\n", g_slist_length(filenames));
			filenames = g_slist_remove(filenames, g_slist_nth_data(filenames, 0));
		//	printf("filename length after remove is %d\n", g_slist_length(filenames));
			filenames = g_slist_append(filenames, (gpointer)strdup(to_name));
		//	printf("filename length after appending is %d\n", g_slist_length(filenames));

		} else
			printf("Unlikely to happend\n");

		//printf("filename length is %d\n", g_slist_length(filenames));
		//g_slist_foreach(filenames, print, NULL);

		close(*fd);
		*fd = open_file(dump_file, WRITE, DUMP_BTSNOOP);
	}
...
static int process_frames(int dev, int sock, int fd, unsigned long flags)
{
	...
	while (1) {
		...
		case WRITE:
			...
			if (write_n(realfd, buf, frm.data_len + hdr_size) < 0) {
				perror("Write error");
				return -1;
			}
			/* tell if filesize exceeds */
			check_filesize(&realfd);
			break;
...
static GKeyFile *load_config(const char *file)
{
	...
	if (!g_key_file_load_from_file(keyfile, file, 0, &err)) {
		if (!g_error_matches(err, G_FILE_ERROR, G_FILE_ERROR_NOENT))
			printf("Parsing %s failed: %s", file, err->message);
		g_error_free(err);
...
static void check_config(GKeyFile *config)
{
	...
	for (i = 0; keys != NULL && keys[i] != NULL; i++) {
		...
		if (!match)
			fprintf(stderr, "Unknown group %s in hcidump.conf", keys[i]);
	}
	...
	for (i = 0; keys != NULL && keys[i] != NULL; i++) {
		...
		if (!found)
			fprintf(stderr, "Unknown key %s in hcidump.conf", keys[i]);
	}
...
int main(int argc, char *argv[])
{
	...
	while ((opt = getopt_long(argc, argv,
		...
		case 'z':
			...
			main_conf = load_config("/etc/bluetooth" "/hcidump.conf");
