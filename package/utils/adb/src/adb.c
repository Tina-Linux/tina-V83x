...
void handle_packet(apacket *p, atransport *t)
{
    ...
    case A_AUTH:
        D("handle_packet() AUTH with arg:%u\n", p->msg.arg0);
...
int adb_main(int is_daemon, int server_port)
{
    ...
    auth_value = getenv("ADB_AUTH_ENABLE");
    if (auth_value != NULL) {
        auth_enabled = !strcmp(auth_value, "1");
    }
#endif
    if (auth_enabled) {
        char *key_path = NULL;
        if (external_key_path != NULL) {
            free(external_key_path);
            external_key_path = NULL;
        }
        key_path = getenv("ADB_AUTH_EXTERNAL_KEY_PATH");
        if (key_path != NULL) {
            external_key_path = strdup(key_path);
            D("ADB_AUTH_EXTERNAL_KEY_PATH=%s\n", external_key_path);
        }
        adb_auth_init();
    }
    ...
    port_value = getenv("ADB_TRANSPORT_PORT");
    if (port_value != NULL) {
	strncpy(value, port_value, strlen(port_value)+1);
    }
#endif
