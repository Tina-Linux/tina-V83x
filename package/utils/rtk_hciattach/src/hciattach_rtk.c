...
#define RTK_VERSION "3.1.0099684.20181218-163903"
...
int h5_vendor_change_speed(int fd, uint32_t baudrate)
{
	...
	int result;
	uint32_t _baudrate = 150000;
	cmd[0] = 0x17;
	cmd[1] = 0xfc;
	cmd[2] = 4;

	_baudrate = cpu_to_le32(baudrate);
#ifdef BAUDRATE_4BYTES
	memcpy((uint16_t *) & cmd[3], &_baudrate, 4);
#else
	memcpy((uint16_t *) & cmd[3], &_baudrate, 2);
