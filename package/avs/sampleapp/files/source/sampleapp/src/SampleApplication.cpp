...
bool SampleApplication::initialize(
    ...
    auto platform = AW::PlatformAdapter::create((pathToInputFolder+"/config.json").data());
    ...
    /*
     * Creating the LED component that observes various components and prints to the console accordingly.
     */
    auto ledringManager = LedRingManager::create(platform->getShowManager());
    ...
    std::shared_ptr<alexaClientSDK::defaultClient::DefaultClient> client =
        alexaClientSDK::defaultClient::DefaultClient::create(
            ...
            {userInterfaceManager, ledringManager},
            {userInterfaceManager, ledringManager},
    ...
    client->addSpeakerManagerObserver(ledringManager);
...
#ifdef ENABLE_ESP
    buffer = std::make_shared<alexaClientSDK::avsCommon::avs::AudioInputStream::Buffer>(bufferSize);
    std::shared_ptr<alexaClientSDK::avsCommon::avs::AudioInputStream> espDataStream =
        alexaClientSDK::avsCommon::avs::AudioInputStream::create(buffer, WORD_SIZE, MAX_READERS);
...
#else
    std::shared_ptr<alexaClientSDK::avsCommon::avs::AudioInputStream> espDataStream = nullptr;
#endif
    ...
    if (!micWrapper) {
        ACSDK_CRITICAL("Failed to create PortAudioMicrophoneWrapper!");
        return false;
    }
#else
    auto micWrapper =
            alexaClientSDK::sampleApp::AlsaMicrophoneWrapper::create(sharedDataStream, espDataStream, platform);

    if (!micWrapper) {
        ACSDK_CRITICAL(LX("Failed to create AlsaMicrophoneWrapper!"));
        return false;
    }
...
#ifdef ENABLE_ESP
    alexaClientSDK::capabilityAgents::aip::AudioProvider espAudioProvider(
        espDataStream,
        compatibleAudioFormat,
        alexaClientSDK::capabilityAgents::aip::ASRProfile::NEAR_FIELD,
        wakeAlwaysReadable,
    ...
    std::shared_ptr<esp::ESPDataProviderInterface> espProvider = esp::ESPDataProvider::create(espAudioProvider);
    ...
    if(strcmp(detector, "tutudetect") == 0) {
        m_keywordDetector = kwd::TutuClearKeywordDetector::create(
            platform,
            sharedDataStream,
            compatibleAudioFormat,
            {keywordObserver},
    ...
    auto buttonInputManager = alexaClientSDK::sampleApp::ButtonInputManager::create(m_interactionManager, platform);
    if (!buttonInputManager) {
        ACSDK_CRITICAL(LX("Failed to create ButtonInputManager!"));
        return false;
    }
    return true;
}
