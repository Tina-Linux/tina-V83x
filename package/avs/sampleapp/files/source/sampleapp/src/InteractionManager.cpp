...
InteractionManager::InteractionManager(
    ...
    std::shared_ptr<sampleApp::MicrophoneWrapperInterface> micWrapper,
...
void InteractionManager::beginHoldToTalk() {
    m_executor.submit([this]() {
        if (!m_isMicOn) {
            return;
        }
...
void InteractionManager::endHoldToTalk() {
    m_executor.submit([this]() {
        if (!m_isMicOn) {
            return;
        }
...
void InteractionManager::beginPushToTalk() {
    m_executor.submit([this]() {
        if (!m_isMicOn) {
            return;
        }
...
void InteractionManager::setVolume(avsCommon::sdkInterfaces::SpeakerInterface::Type type, uint8_t volume) {
    m_executor.submit([this, type, volume]() {
        /*
        ...
        std::future<bool> future = m_client->getSpeakerManager()->setVolume(type, volume);
        if (!future.valid()) {
            return;
        }
        future.get();
