...
/// The keyword model data that has been compiled in externally defined in the model.cpp file.
extern unsigned char prlBinaryModelData[];

/// The total size of the model binary in bytes externally defined in the model.cpp file.
 ...
 * @param return @c true if the model was successfully loaded into memory and @c false otherwise.
 */
...
// Map of Decoder instance to detector objects. This should be locked with g_lock before access.
...
static bool isAudioFormatCompatibleWithPryonLite(avsCommon::utils::AudioFormat audioFormat) {
    if (audioFormat.numChannels != PRYON_LITE_COMPATIBLE_NUM_CHANNELS) {
        ACSDK_ERROR(LX("isAudioFormatCompatibleWithPryonLiteFailed")
                        .d("reason",
                           "Audio data number of channels does not meet PryonLite requirements of " +
    ...
    if (audioFormat.sampleRateHz != PRYON_LITE_COMPATIBLE_SAMPLE_RATE_HZ) {
        ACSDK_ERROR(LX("isAudioFormatCompatibleWithPryonLiteFailed")
                        .d("reason",
                           "Audio data sample rate does not meet PryonLite requirements of " +
    ...
    if (audioFormat.sampleSizeInBits != PRYON_LITE_COMPATIBLE_SAMPLE_SIZE_IN_BITS) {
        ACSDK_ERROR(LX("isAudioFormatCompatibleWithPryonLiteFailed")
                        .d("reason",
                           "Audio data bits per sample does not meet PryonLite requirements of " +
...
void PryonLiteKeywordDetector::resultCallback(PryonLiteDecoderHandle handle, const PryonLiteResult* result) {
    ...
    detector->notifyKeyWordObservers(
        detector->m_stream, result->keyword, result->beginSampleIndex, result->endSampleIndex, KWDMetadata);
}
...
bool PryonLiteKeywordDetector::init(
    ...
    m_streamReader = m_stream->createReader(avsCommon::avs::AudioInputStream::Reader::Policy::BLOCKING);
    if (!m_streamReader) {
    ...
    if (!pathToModelFile.empty()) {
        ACSDK_WARN(LX("init").d("warning", "model file path specified when built in static model compliation mode"));
    }
#else
...
void PryonLiteKeywordDetector::detectionLoop() {
    ...
    while (!m_isShuttingDown) {
        ...
        /*
         * PryonLite requires that m_pryonLiteSamplesPerPush samples are pushed per call to
         * PryonLiteDecoder_PushAudioSamples(). Anything less than that causes issues with proper detection.
         */
        while (wordsReadSoFar < m_pryonLiteSamplesPerPush && !m_isShuttingDown) {
            bool didErrorOccur = false;
            size_t wordsReadThisIteration = 0;
