...
bool MediaPlayer::init() {
    if (false == gst_init_check(nullptr, nullptr, nullptr)) {
        ...
        case GST_MESSAGE_STATE_CHANGED: {
            ...
            if (GST_MESSAGE_SRC(message) == GST_OBJECT_CAST(m_pipeline.pipeline)) {
                ...
                if (GST_STATE_READY == newState) {
                    ...
                    // Reset sync state back to true if tsdemux changes to nullptr state
...
void MediaPlayer::handlePlay(SourceId id, std::promise<bool>* promise) {
    ...
    GstState curState;
    auto stateChange = gst_element_get_state(m_pipeline.pipeline, &curState, nullptr, TIMEOUT_ZERO_NANOSECONDS);
...
void MediaPlayer::handlePause(MediaPlayer::SourceId id, std::promise<bool>* promise) {
    ...
    GstState curState;
    auto stateChangeRet = gst_element_get_state(m_pipeline.pipeline, &curState, nullptr, TIMEOUT_ZERO_NANOSECONDS);
    if (GST_STATE_CHANGE_FAILURE == stateChangeRet) {
...
void MediaPlayer::handleGetOffset(SourceId id, std::promise<std::chrono::milliseconds>* promise) {
    ...
    auto stateChangeRet = gst_element_get_state(m_pipeline.pipeline, &state, nullptr, TIMEOUT_ZERO_NANOSECONDS);
