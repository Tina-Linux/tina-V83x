...
int main(int argc, char **argv)
{
	...
	TTSMPL					atTSMPLAECRef[FRAMESZ_4_TUTU*2];
	TTSMPL					atTSMPLMicSig[FRAMESZ_4_TUTU*NO_MIC_INPUT]; // up to 8 channels
	TTSMPL					atTSMPLLOut[FRAMESZ_4_WAKEUP];
	FILE                    *pfLout = NULL;
	...
	memset(atTSMPLAECRef, 0, FRAMESZ_4_TUTU*sizeof(TTSMPL));
	...
	while (!should_stop)
	{
		...
		while (!done)
		{
			...
			for (k=0; k<NO_MIC_INPUT; k++)
			{
				...
				w32SampleValue = w32SampleValue & (W32)0xFFFFF000;
					...
					if (w32TargetChn < 6)
						atTSMPLMicSig[FRAMESZ_4_TUTU*w32TargetChn+l] = w32SampleValue;
					else
						atTSMPLAECRef[FRAMESZ_4_TUTU*(w32TargetChn-6)+l] = w32SampleValue;
				}
			}
			...
			// place NS result to wakeup 16-b input port
			//memcpy(p->audio, atTSMPLLOut, uw32FrameSzInSampleForWakeUp*sizeof(short));
			for (l=0; l<uw32FrameSzInSampleForWakeUp; l++)
				p->audio[l] = (W16)(atTSMPLLOut[l] >> 16);

			// if use Sensory 32-b input port (p->audio is 32-b buffer)
			//memcpy(p->audio, atTSMPLLOut, uw32FrameSzInSampleForWakeUp*sizeof(int));
