...
// ----------------------------------------------------------------------------
void TUTUClear_ParsePRMFile_QACT(char			*pszCtrlFileName,
...
int main(int argc, char **argv)
{
	...
	pw8CapBuf = malloc(uw32FrameSzInSampleForWakeUp*NO_MIC_INPUT*2);
	if (!pw8CapBuf)
	{
		printf("Unable to allocate %d bytes\n", uw32FrameSzInSampleForWakeUp*NO_MIC_INPUT*2);
		free(pw8CapBuf);
	}
	...
	while (!should_stop)
	{
		...
		while (!done)
		{
			...
			pcm_read(pcm, pw8CapBuf, uw32FrameSzInSampleForWakeUp*NO_MIC_INPUT*sizeof(short));
			...
			// frame size needs to be 10 ms
			{
				...
				for (k=0; k<NO_MIC_INPUT; k++)
				{
					...
					{
						int		w32TargetChn;
						short	w16SampleValue = ((short *)pw8CapBuf)[l*NO_MIC_INPUT+k];

						// compensate channel shuffle
						w32TargetChn = (W32)w16SampleValue & (W32)0x000F;
						w16SampleValue = w16SampleValue & 0xFFF0;
...
TIniFileInfo *OpenIniFile(char *pszFileName)
{
	FILE            *fptr;
...
//---------------------------------------------------------------------------
int ReadWord32(TIniFileInfo *pIniFileInfo, char *pszIdent, W32 *pw32Value, W32 w32Default)
{
	int i;
	TIdentValue *pIdentValue = pIniFileInfo->pIdentValueList;
...
//---------------------------------------------------------------------------
int ReadUWord32(TIniFileInfo *pIniFileInfo, char *pszIdent, UW32 *puw32Value, UW32 uw32Default)
{
	int i;
	TIdentValue *pIdentValue = pIniFileInfo->pIdentValueList;
