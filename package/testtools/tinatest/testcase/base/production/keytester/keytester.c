...
inline int judgment_duplicate_values(int *tmp,int value,int count)
{
	int i;
	for(i = 0;i < count; i++) {
		if(tmp[i] == value)
...
int main(int argc, char *argv[])
{
    ...
    pt_key_value=(int *)malloc(number_of_keys * sizeof(int));
    if(NULL == pt_key_value){
	    printf("Memory allocation failed!\n");
	    goto err;
    }
    strcpy(key_name, "sunxi-keyboard");
    ...
    for (i = 0; i < 5; i++) {
	    ...
	    //lradc key
	    if (strncmp(evt_path, key_name, strlen(key_name)) == 0) {
		    fclose(fp);
		    break;
	    }

	    //gpadc key
	    if (NULL != strstr(evt_path, "sunxi-gpadc")) {
		    fclose(fp);
		    break;
	    }
	    fclose(fp);
    ...
    if (i == 5) {
        printf("NO!!! keyboard input found");
        goto err;
    }
