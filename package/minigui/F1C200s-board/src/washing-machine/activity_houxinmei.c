...
static DoubleTextView* DoubleTextDataInit(DoubleTextView *doubleTextData,
	...
	case 2:
		...
		doubleTextData->valueUnitDes[3] = "60";
		doubleTextData->valueUnitDes[4] = DU;
		doubleTextData->valueUnitDes[5] = WENDU;

		doubleTextData->enableClick[0] = ENABLE_CLICK_YES;
		doubleTextData->enableClick[1] = ENABLE_CLICK_NO;
		break;
	case 3:
		doubleTextData->valueUnitDes[0] = "60";
...
static void my_notif_proc(HWND hwnd, int id, int nc, DWORD add_data) {
	if (nc == STN_CLICKED) {
		features_menu(hwnd, 7);
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityHouXinMeiProc(HWND hWnd, int message, WPARAM wParam,
		LPARAM lParam) {
	...
	case MSG_CREATE: {
		setCurrentIconValue(ID_FUN_SCREEN_BG2, 0);
	...
	case MSG_PAINT: {
		...
		rcClient.right = 400;
		rcClient.bottom = 400;
		DrawText(hdc, HUOXINGMEI_N, -1, &rcClient,
...
int ActivityHouXinMei(HWND hosting, HeadBarView *myHeadBarData) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityHouXinMeiProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
