...
static void openWin(HWND hWnd, int index) {
	...
	case 2:
		freedSta = 1;
		PostMessage(hWnd, MSG_CLOSE, 0, 0);
		PostMessage(GetHosting(hWnd), MSG_TAKE_CAMERA_SETUP, 2, 0);
		break;
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityTVideoProc(HWND hWnd, int nMessage, WPARAM wParam,
		LPARAM lParam) {
...
int ActivityTVideo(HWND hosting) {
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityTVideoProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
