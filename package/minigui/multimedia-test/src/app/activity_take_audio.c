...
/* Microphone initialization status, 0 success, 1 failed */
...
/* Initialize the microphone, using only the front microphone */
...
static void openWin(HWND hWnd, int index) {
	...
	case 2:
		freedSta = 1;
		PostMessage(hWnd, MSG_CLOSE, 0, 0);
		PostMessage(GetHosting(hWnd), MSG_TAKE_CAMERA_SETUP, 1, 0);
		break;
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityTAudioProc(HWND hWnd, int nMessage, WPARAM wParam,
		LPARAM lParam) {
...
int ActivityTAudio(HWND hosting) {
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityTAudioProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
