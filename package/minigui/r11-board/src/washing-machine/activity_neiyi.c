...
static SlideTextView* SlideTextDataInit(SlideTextView *slideTextData, int num) {
	...
	switch (num) {
	case 1:
		slideTextData->listData1[0] = "19";
		slideTextData->listData1[1] = "24";
		slideTextData->listData1[2] = "29";
		slideTextData->listData1[3] = "34";
		slideTextData->listData1[4] = "39";
...
static void my_notif_proc(HWND hwnd, int id, int nc, DWORD add_data) {
	if (nc == STN_CLICKED) {
		features_menu(hwnd, 9);
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityNeiYiProc(HWND hWnd, int message, WPARAM wParam,
		LPARAM lParam) {
	...
	case MSG_TIMER: {
		...
		headbarbtnwifi[9] = HeadBarViewInit(hWnd, bottomBar, headBarData);

		SendMessage(headbarbtnwifi[9], MSG_UPDATETIME, NULL, NULL);
	...
	case MSG_CREATE: {
		setCurrentIconValue(ID_FUN_SCREEN_BG2, 3);
	...
	case MSG_PAINT: {
		...
		rcClient.right = 400;
		rcClient.bottom = 400;
		DrawText(hdc, NEIYI_N, -1, &rcClient,
...
int ActivityNeiYi(HWND hosting, HeadBarView *myHeadBarData) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityNeiYiProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
