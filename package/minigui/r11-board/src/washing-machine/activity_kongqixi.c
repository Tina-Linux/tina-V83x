...
static void my_notif_proc(HWND hwnd, int id, int nc, DWORD add_data) {
	if (nc == STN_CLICKED) {
		features_menu(hwnd, 16);
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityKongQiXiProc(HWND hWnd, int message, WPARAM wParam,
		LPARAM lParam) {
	...
	case MSG_TIMER: {
		...
		headbarbtnwifi[16] = HeadBarViewInit(hWnd, bottomBar, headBarData);

		SendMessage(headbarbtnwifi[16], MSG_UPDATETIME, NULL, NULL);
	...
	case MSG_CREATE: {
		setCurrentIconValue(ID_FUN_SCREEN_BG3, 4);
	...
	case MSG_PAINT: {
		...
		rcClient.right = 400;
		rcClient.bottom = 400;
		DrawText(hdc, KONGQIXI_N, -1, &rcClient,
...
int ActivityKongQiXi(HWND hosting, HeadBarView *myHeadBarData) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityKongQiXiProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
