...
static SlideTextView* SlideTextDataInit(SlideTextView *slideTextData, int num) {
	...
	switch (num) {
	case 1:
		slideTextData->listData1[0] = "22";
		slideTextData->listData1[1] = "27";
		slideTextData->listData1[2] = "32";
		slideTextData->listData1[3] = "37";
		slideTextData->listData1[4] = "42";
...
static void my_notif_proc(HWND hwnd, int id, int nc, DWORD add_data) {
	if (nc == STN_CLICKED) {
		features_menu(hwnd, 10);
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityYuRonFuProc(HWND hWnd, int message, WPARAM wParam,
		LPARAM lParam) {
	...
	case MSG_TIMER: {
		...
		headbarbtnwifi[10] = HeadBarViewInit(hWnd, bottomBar, headBarData);

		SendMessage(headbarbtnwifi[10], MSG_UPDATETIME, NULL, NULL);
	...
	case MSG_CREATE: {
		setCurrentIconValue(ID_FUN_SCREEN_BG2, 4);
	...
	case MSG_PAINT: {
		...
		rcClient.right = 400;
		rcClient.bottom = 400;
		DrawText(hdc, YURONGFU_N, -1, &rcClient,
...
int ActivityYuRonFu(HWND hosting, HeadBarView *myHeadBarData) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityYuRonFuProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
