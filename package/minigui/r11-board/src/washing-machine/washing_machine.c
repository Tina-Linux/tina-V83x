...
//	WinDataType *mwd;
	...
	case MSG_HOME_CLICK: {
		...
		if (strcmp((char*) lParam, WIN_DES_MAIN) == 0) {
			...
			//reload picture
			//loadManyViewBitMap();
		    /* Open double buffer to ensure that the main window slides without tangents */
		    DoubleBufferEnable(TRUE);
			ManyWinAppear();
	...
	case MSG_MANY_WIN_CLICK: {
		if (!isOpenOtherActivity) {
			...
			//free the memory
			//unloadManyViewBitMap();
			/* Turn off double buffer, in order to make headbar and bottombar appear smoother */
...
int MiniGUIMain(int argc, const char* argv[])
{
	...
	if (g_win_data->ParentHwnd == HWND_INVALID)
		return -1;
	printf("g_win_data->ParentHwnd=%d\n", g_win_data->ParentHwnd);
	...
	UnregisterSwipeView();
	UnregisterLaundryView();
	UnregisterLaundryPartView();
