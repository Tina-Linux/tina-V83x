...
static SlideTextView* SlideTextDataInit(SlideTextView *slideTextData, int num) {
	...
	switch (num) {
	case 1:
		slideTextData->listData1[0] = "14";
		slideTextData->listData1[1] = "17";
		slideTextData->listData1[2] = "22";
		slideTextData->listData1[3] = "27";
		slideTextData->listData1[4] = "32";
...
static void my_notif_proc(HWND hwnd, int id, int nc, DWORD add_data) {
	if (nc == STN_CLICKED) {
		features_menu(hwnd, 2);
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityHunHeXiProc(HWND hWnd, int message, WPARAM wParam,
		LPARAM lParam) {
	...
	case MSG_TIMER: {
		...
		headbarbtnwifi[2] = HeadBarViewInit(hWnd, bottomBar, headBarData);

		SendMessage(headbarbtnwifi[2], MSG_UPDATETIME, NULL, NULL);
	...
	case MSG_CREATE: {
		setCurrentIconValue(ID_FUN_SCREEN_BG1, 2);
	...
	case MSG_PAINT: {
		...
		rcClient.right = 400;
		rcClient.bottom = 400;
		DrawText(hdc, HUNHEXI_N, -1, &rcClient,
...
int ActivityHunHeXi(HWND hosting, HeadBarView *myHeadBarData) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityHunHeXiProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
