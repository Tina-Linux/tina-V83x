...
static int ActivityYanMaoProc(HWND hWnd, int message, WPARAM wParam,
		LPARAM lParam) {
	...
	case MSG_TIMER: {
		...
		headbarbtnwifi[7] = HeadBarViewInit(hWnd, bottomBar, headBarData);

		SendMessage(headbarbtnwifi[7], MSG_UPDATETIME, NULL, NULL);
	...
	case MSG_CREATE: {
		setCurrentIconValue(ID_FUN_SCREEN_BG2, 1);
	...
	case MSG_PAINT: {
		...
		rcClient.right = 400;
		rcClient.bottom = 400;
		DrawText(hdc, YANGMAO_N, -1, &rcClient,
...
int ActivityYanMao(HWND hosting, HeadBarView *myHeadBarData) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityYanMaoProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
