...
static SlideTextView* SlideTextDataInit(SlideTextView *slideTextData, int num) {
	...
	switch (num) {
	case 1:
		slideTextData->listData1[0] = "37";
		slideTextData->listData1[1] = "42";
		slideTextData->listData1[2] = "52";
		slideTextData->listData1[3] = "57";
		slideTextData->listData1[4] = "62";
...
static void my_notif_proc(HWND hwnd, int id, int nc, DWORD add_data) {
	if (nc == STN_CLICKED) {
		features_menu(hwnd, 4);
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityDaJianProc(HWND hWnd, int message, WPARAM wParam,
		LPARAM lParam) {
	...
	case MSG_TIMER: {
		...
		headbarbtnwifi[4] = HeadBarViewInit(hWnd, bottomBar, headBarData);

		SendMessage(headbarbtnwifi[4], MSG_UPDATETIME, NULL, NULL);
	...
	case MSG_CREATE: {
		setCurrentIconValue(ID_FUN_SCREEN_BG1, 4);
	...
	case MSG_PAINT: {
		...
		rcClient.right = 400;
		rcClient.bottom = 400;
		DrawText(hdc, DAJIAN_N, -1, &rcClient,
...
int ActivityDaJian(HWND hosting, HeadBarView *myHeadBarData) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityDaJianProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
