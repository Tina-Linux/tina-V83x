...
#define MEDIA_PARTH_A  "/mnt/exUDISK"
...
int zw_sort_list(list_head_t *head)
{
	...
	static int countfirstnode1 = 0;//用来记录第一个文件是否为"ZW"文件
	s32 total;
	char name1[64];
	char name11[64];
	...
	//遍历链表，把"ZW"格式的文件放到第一个不为"ZW"格式文件的前面
	temp_node = temp_node->next;
...
int getMediaFile(void)
{
    ...
    char ininame[128] = {"/usr/res/networkres/mediafile.txt"};
...
int create_media_list_player(void)
{
	...
	while((dir_info = readdir(dir)) != NULL){
                ...
                if(strcmp(dir_info->d_name+strlen(dir_info->d_name)-4, ".mp4") == 0) {
                        ...
                        snprintf(compete_path,FILE_PATH_MAXT_LEN, "%s/%s", MEDIA_PARTH_A, dir_info->d_name);
...
int create_media_list(void)
{
	...
	//sort_list(media_list);		// 按时间排序
	//zw_sort_list(media_list);	//  把zw文件放在前面
...
int show_current_file(void)
{
	...
	if(current_node->type == MEDIA_F_TYPE_VIDEO){
		...
		TRetrieverSetDataSource(metedataretriever, current_node->path,TMETADATA_RETRIEVER_SCALE_DOWN_2, TmetadataRetrieverOutputDataYV12);
		VideoFrame* videoFrame = TRetrieverGetFrameAtTime(metedataretriever, -1);//0
		if(videoFrame == NULL){
		...
		disp_out_port->writeData(disp_out_port, videoFrame->mYuvData, videoFrame->mYuvSize, &vparam);
		show_f = 1;
	}else{
		//播放图片
		...
		JpegDecoderSetDataSource(jpegdecoder, current_node->path, JPEG_DECODE_SCALE_DOWN_4, JpegDecodeOutputDataYV12);
		...
		if(imgFrame == NULL){
			...
			printf("JpegDecoderGetFrame successfully,imgFrame->mDisplayWidth = %d,imgFrame->mDisplayHeight = %d,imgFrame->mYuvData = %p,imgFrame->mYuvSize = %d\n", \
				imgFrame->mDisplayWidth, imgFrame->mDisplayHeight,imgFrame->mYuvData,imgFrame->mYuvSize);
		...
		vparam.srcInfo.color_space = DISP_BT601;
		disp_out_port->allocateVideoMem(disp_out_port, &vparam);
		disp_out_port->writeData(disp_out_port, imgFrame->mYuvData, imgFrame->mYuvSize, &vparam);
		show_f = 2;
	}
