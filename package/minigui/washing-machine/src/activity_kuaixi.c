...
static DoubleTextView* DoubleTextDataInit(DoubleTextView *doubleTextData,
	...
	switch (num) {
	case 1:
		doubleTextData->valueUnitDes[0] = "2";
	...
	case 2:
		...
		break;
	case 3:
		doubleTextData->valueUnitDes[0] = LENGSHUI;
...
static SlideTextView* SlideTextDataInit(SlideTextView *slideTextData, int num) {
	...
	case 2:
		...
		break;
	case 3:
		slideTextData->listData1[0] = LENGSHUI;
		slideTextData->listData1[1] = "20";
		slideTextData->listData1[2] = "30";
		slideTextData->listData1[3] = "40";
...
static void my_notif_proc(HWND hwnd, int id, int nc, DWORD add_data) {
	if (nc == STN_CLICKED) {
		features_menu(hwnd, 3);
	}
}

#define __ID_TIMER_SLIDER 100
static int ActivityKuaiXiProc(HWND hWnd, int message, WPARAM wParam,
		LPARAM lParam) {
	...
	case MSG_CREATE: {
		setCurrentIconValue(ID_FUN_SCREEN_BG1, 3);
	...
	case MSG_PAINT: {
		...
		rcClient.right = 400;
		rcClient.bottom = 400;
		DrawText(hdc, KUAIXI_N, -1, &rcClient,
...
int ActivityKuaiXi(HWND hosting, HeadBarView *myHeadBarData) {
	MSG Msg;
	HWND hMainWnd;
	...
	CreateInfo.hIcon = 0;
	CreateInfo.MainWindowProc = ActivityKuaiXiProc;
	CreateInfo.lx = 0;
	CreateInfo.ty = 0;
