	...
	switch(msg) {
		case T_RECORD_ONE_FILE_COMPLETE:
		{
			...
			snprintf(trTestContext->mRecorderPath, sizeof(trTestContext->mRecorderPath),
			            "/mnt/UDISK/AW_%s_video%d.mp4", num[trTestContext->mRecorderId],
			            trTestContext->mRecorderFileCount);
        ...
        case T_RECORD_ONE_AAC_FILE_COMPLETE:
		{
			...
			snprintf(trTestContext->mRecorderPath, sizeof(trTestContext->mRecorderPath),
			            "/mnt/UDISK/AW_%s_audio%d.aac", num[trTestContext->mRecorderId],
			            trTestContext->mRecorderFileCount);
...
static int oneChannelTest(RecoderTestContext *recorderTestContext, RecorderStatusContext *RecorderStatus, int number)
{
    ...
    TRsetMaxRecordTimeMs(recorderTestContext[i].mTrecorder, 5*1000);
