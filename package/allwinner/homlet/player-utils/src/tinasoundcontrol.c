...
int TinaSoundDeviceWrite(SoundCtrl* s, void* pData, int nDataSize){
    ...
    if (muteValue != 0 || (volumeValue / 5 - 20) != 0)
    {
        AudioGain audioGain;
        audioGain.preamp = (muteValue == 0) ? (volumeValue / 5 - 20) : -20;
...
int TinaSoundDeviceSetVolume(SoundCtrl* s, int volume){
    logd("TinaSoundDeviceSetVolume(%d)", volume);
...
int TinaSoundDeviceGetVolume(SoundCtrl* s, int* volume){
    SoundCtrlContext* sc;
    sc = (SoundCtrlContext*)s;
    if(sc)
    {
        ...
        *volume = (muteValue == 0) ? volumeValue : 0;
...
int TinaSoundDeviceSetMute(SoundCtrl* s, int mute){
    logd("TinaSoundDeviceSetMute(%d)", mute);
...
int TinaSoundDeviceGetMute(SoundCtrl* s, int* mute){
    SoundCtrlContext* sc;
    sc = (SoundCtrlContext*)s;
    if(sc)
    {
