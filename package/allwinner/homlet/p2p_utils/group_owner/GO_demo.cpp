...
typedef struct peerDeviceInfo{
   char p2p_dev_addr[18];
   char pri_dev_type[14];
   char name[128];
   int config_methods;
   int dev_capab;
   int group_capab;
   char wfd_dev_info[15];
}peerInfo;

char* parserDeviceInfo()
{
   //todo;
}
...
void* getMacAndName(const char *elem1,char *mac,char *name)
{
	 ...
	 pLeft= strstr(elem1,"p2p_dev_addr=");
	 strncpy(mac,pLeft+13,18);

     if(name)
     {
	    pLeft = strstr(elem1,"name='");
...
{
	wfd_logd("R6 tina p2p demo start");

    int ret = -1;
    char gcElem[] = "000600101c440032";
...
while(count-- > 0)
{
    //wait to search p2p device
    usleep(10*1000*1000);

	// disp avtive p2p device and choice to connect
	for(map<string,string>::const_iterator it = activeDevices.begin();
          it != activeDevices.end(); it++)
    {
         wfd_logd("[want connect p2p wfdtest] name: %s, mac: %s",it->second.c_str(),it->first.c_str());

		 //connect p2p device which one you like;
	     if(strstr(it->second.c_str(),"wfdtest")
		 	|| strstr(it->second.c_str(),"Honor"))
	     {
	        wfd_logd("connect: %s",it->second.c_str());
		    AwP2PConnect(it->first.c_str());
	     }
    }

	wfd_logd("do communication");
	usleep(30*1000*1000); // do communication

    //disconnect; go disconnect gc or gc disconnect go;
	//peer WFD_CB_DEVICE_LOST atuomatic
...
//----------------------------
	...
	wfd_logd("H6 tina p2p group owner demo over");
