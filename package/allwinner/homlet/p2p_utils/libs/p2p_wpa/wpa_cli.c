...
static wfdP2PCallback wfdP2PCB = NULL;
static char peer_dev_addr[18] = "";
...
static char ctrl_ifname_active[100] = "";
...
{
	char buf[4096]; // wpa_supplicant's REPLY_BUF_SIZE maximum size is 4096.
	size_t len;
	int ret;
...
int wpa_cli_cmd_with_reply(const char *cmd, char *reply, size_t *reply_len)
{
	...
	ret = wpa_ctrl_request(ctrl_conn, cmd, os_strlen(cmd), reply, reply_len, NULL);
	if (ret == -2) {
		printf("'%s' command timed out.\n", cmd);
		return -1;
	} else if (ret < 0) {
...
static void wpa_cli_action_process(const char *msg)
{
	...
	if (str_starts(pos, WPA_EVENT_CONNECTED)) {
		...
		printf("receive: %s\n", pos);
		//get active pep interface
		char *start = strstr(pos, "p2p");
		if (start) {
			char *end = strstr(start, " ");
			if (end && (unsigned int) (end - pos) < sizeof(ctrl_ifname_active)) {
				os_memcpy(ctrl_ifname_active, start, end - start);
				ctrl_ifname_active[end - start] = '\0';
			}
		}
		printf("get interface: %s \n",ctrl_ifname_active);
		int ret = wpa_cli_exec("/usr/bin/p2p_action.sh", ifname, pos);
		printf("/usr/bin/p2p_action.sh ret: %d\n", ret);
		...
		if (argc < 8) {
			printf("wrong P2P-GROUP-STARTED\n");
			return;
		}
		char *go_or_client = argv[2];
		printf("P2P-GROUP-STARTED: %s\n", go_or_client);
		char *deviceAddress = NULL;
		char go_dev_addr[18] = "";
		if (os_strcmp(go_or_client, "client") == 0) {
			pos = os_strstr(pos, "go_dev_addr=");
			pos += 12;
			const char *end = os_strchr(pos, ' ');
			if (end && (unsigned int) (end - pos) < sizeof(go_dev_addr)) {
				os_memcpy(go_dev_addr, pos, end - pos);
				go_dev_addr[end - pos] = '\0';
				printf("P2P-GROUP-STARTED: %s\n", go_dev_addr);
				deviceAddress = go_dev_addr;
			}
		} else if (os_strcmp(go_or_client, "GO") == 0) {
			printf("P2P-GROUP-STARTED: %s\n", peer_dev_addr);
			deviceAddress = peer_dev_addr;
		} else {
			printf("wrong P2P-GROUP-STARTED\n");
		}

		if (deviceAddress && os_strcmp(deviceAddress, "") != 0) {
			int ret = -1;
			char reply[4096] = "";
			size_t reply_len;
			char p2p_peer_cmd[4096] = "";
			argc = 0;
			argv[argc++] = deviceAddress;
			reply_len = sizeof(reply);
			write_cmd(p2p_peer_cmd, sizeof(p2p_peer_cmd), "P2P_PEER", argc, argv);
			ret = wpa_cli_cmd_with_reply(p2p_peer_cmd, reply, &reply_len);
			printf("%s: %s\n", p2p_peer_cmd, reply);
			int tryCount = 0;
			while (wpa_cli_quit == 0 && ret >= 0 && tryCount < 5
				&& (os_strncmp(reply, "PONG", 4) == 0 || os_strncmp(reply, "FAIL", 4) == 0
					|| os_strncmp(reply, "OK", 2) == 0)) {
			...
			if (ret >= 0 && os_strncmp(reply, "PONG", 4) != 0 && os_strncmp(reply, "FAIL", 4) != 0
				&& os_strncmp(reply, "OK", 2) != 0) {
				printf("%s: will parse wfd_subelems\n", p2p_peer_cmd);
				const char *wfd_subelems = os_strstr(reply, "wfd_subelems=");
				char deviceInfo[5] = "";
				char ctrlPort[5] = "";
				char maxThroughput[5] = "";
				sscanf(wfd_subelems, "wfd_subelems=000006%4s%4s%4s", deviceInfo, ctrlPort, maxThroughput);
				int ctrlPortInt = strtol(ctrlPort, NULL, 16);
				printf("P2P-GROUP-STARTED: %s, %s, %d\n", go_or_client, deviceAddress, ctrlPortInt);
				if (wfdP2PCB) {
					if (os_strcmp(go_or_client, "GO") == 0) {
						wfdP2PCB(WFD_CB_CONNECTED_GO, deviceAddress, ctrlPortInt);
					} else if (os_strcmp(go_or_client, "client") == 0) {
						wfdP2PCB(WFD_CB_CONNECTED_CLIENT, deviceAddress, ctrlPortInt);
					}
				}
			}
		}
		...
		os_memset(peer_dev_addr, '\0', sizeof(peer_dev_addr));
		if (wfdP2PCB) wfdP2PCB(WFD_CB_DISCONNECTED, NULL, 0);
		...
		argc = tokenize_cmd(copy, argv);
		if (argc < 2) {
			printf("wrong P2P-GO-NEG-REQUEST\n");
			return;
		}
		char *deviceAddress = argv[1];
		printf("P2P-GO-NEG-REQUEST: %s\n", deviceAddress);
		int devPasswdId = 0;
		if (argc > 2) {
			pos = os_strstr(argv[2], "dev_passwd_id=");
			devPasswdId = atoi(pos + 14);
			printf("P2P-GO-NEG-REQUEST: devPasswdId=%d\n", devPasswdId);
		}
		if (argc > 3) {
			pos = os_strstr(argv[3], "go_intent=");
			int goIntent = atoi(pos + 10);
			printf("P2P-GO-NEG-REQUEST: goIntent=%d\n", goIntent);
		}

		char reply[4096] = "";
		size_t reply_len;

		//char p2p_stop_find_cmd[4096] = "";
		//argc = 0;
		//reply_len = sizeof(reply);
		//write_cmd(p2p_stop_find_cmd, sizeof(p2p_stop_find_cmd), "P2P_STOP_FIND", argc, argv);
		//wpa_cli_cmd_with_reply(p2p_stop_find_cmd, reply, &reply_len);
		//printf("%s: %s\n", p2p_stop_find_cmd, reply);
		...
		if (ret >= 0 && os_strncmp(reply, "PONG", 4) != 0 && os_strncmp(reply, "FAIL", 4) != 0
			&& os_strncmp(reply, "OK", 2) != 0) {
			printf("%s: will parse group_capab\n", p2p_peer_cmd);
			const char *group_capab = os_strstr(reply, "group_capab=");
			groupCapability = strtol(group_capab + 12, NULL, 16);
		}

		char p2p_connect_cmd[4096] = "";
		argc = 0;
		argv[argc++] = deviceAddress;
		argv[argc++] = "pbc";
		...
		if ((groupCapability & 1) != 0) {
			...
			argv[argc++] = "go_intent=6";
		}
		reply_len = sizeof(reply);
		write_cmd(p2p_connect_cmd, sizeof(p2p_connect_cmd), "P2P_CONNECT", argc, argv);
		wpa_cli_cmd_with_reply(p2p_connect_cmd, reply, &reply_len);
		printf("%s: %s\n", p2p_connect_cmd, reply);

		os_free(copy);
	} else if (str_starts(pos, P2P_EVENT_PROV_DISC_PBC_REQ)) {
		printf("receive %s: %s\n", ifname, pos);
		// follow android framework, do nothing
		return;
		...
		argc = tokenize_cmd(copy, argv);
		if (argc < 8) {
			printf("wrong P2P-PROV-DISC-PBC-REQ\n");
			return;
		}
		char *deviceAddress = argv[1]; // TODO, p2p_dev_addr
		printf("P2P-PROV-DISC-PBC-REQ: %s\n", deviceAddress);

		pos = os_strstr(pos, "config_methods="); // avoid name with space
		int configMethods = strtol(pos + 15, NULL, 16);
		printf("P2P-PROV-DISC-PBC-REQ: configMethods=0x%x\n", configMethods);
		if ((configMethods & 0x80) == 0) {
			printf("P2P-PROV-DISC-PBC-REQ: configMethods unsupport pbc\n");
			return;
		}
		pos = os_strstr(pos, "group_capab=");
		int groupCapability = strtol(pos + 12, NULL, 16);
		printf("P2P-PROV-DISC-PBC-REQ: groupCapability=0x%x\n", groupCapability);
		...
		os_memset(peer_dev_addr, '\0', sizeof(peer_dev_addr));
		pos = os_strstr(pos, "peer_dev=");
		pos += 9;
		const char *end = os_strchr(pos, ' ');
		if (end && (unsigned int) (end - pos) < sizeof(peer_dev_addr)) {
			os_memcpy(peer_dev_addr, pos, end - pos);
			peer_dev_addr[end - pos] = '\0';
			printf("P2P-GO-NEG-SUCCESS: %s\n", peer_dev_addr);
		}
	} else if (str_starts(pos, P2P_EVENT_INVITATION_ACCEPTED)) {
		printf("receive %s: %s\n", ifname, pos);

		os_memset(peer_dev_addr, '\0', sizeof(peer_dev_addr));
		pos = os_strstr(pos, "sa=");
		pos += 3;
		const char *end = os_strchr(pos, ' ');
		if (end && (unsigned int) (end - pos) < sizeof(peer_dev_addr)) {
			os_memcpy(peer_dev_addr, pos, end - pos);
			peer_dev_addr[end - pos] = '\0';
			printf("P2P-INVITATION-ACCEPTED: %s\n", peer_dev_addr);
		}
	} else if (str_starts(pos, WPA_EVENT_TERMINATING)) {
		printf("wpa_supplicant is terminating - stop monitoring\n");
		wpa_cli_quit = 1;
    } else if (str_starts(pos, P2P_EVENT_DEVICE_FOUND)) {
       wfdP2PCB(WFD_CB_DEVICE_FOUND, pos,NULL);
    } else if (str_starts(pos, P2P_EVENT_DEVICE_LOST)) {
       wfdP2PCB(WFD_CB_DEVICE_LOST, pos,0);
    } else if (str_starts(pos, "CTRL-EVENT-")) {
    } else {
	printf("%s: %s, ignore\n", ifname, pos);
    }
}
...
static void wpa_cli_recv_pending(struct wpa_ctrl *ctrl, int action_monitor)
{
	...
	while (wpa_ctrl_pending(ctrl) > 0) {
		char buf[4096]; // wpa_supplicant's EVENT_BUF_SIZE maximum size is 2048.
...
static void wpa_cli_mon_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	wpa_cli_recv_pending(mon_conn, 1);
}
...
{
    if(strcmp(ctrl_ifname_active,"") == 0)
		strcpy(ctrl_ifname_active,"p2p-p2p0-0");

    if(ifname)
	    strcpy(ifname,ctrl_ifname_active); 

    printf("wpa_cli_get_ifname:  %s\n", ctrl_ifname_active);

    return ctrl_ifname_active;
}
...
{
	return (void*)ctrl_conn;
}

void* wpa_cli_get_mon_conn()
{
	return (void*)mon_conn;
}
...
int wpa_cli_cmd_handler(char *cmd)
{
	...
	if (argc) {
		int tryCount = 0;
		while (wpa_cli_quit == 0 && ret < 0 && tryCount < 5) {
			ret = wpa_request(ctrl_conn, argc, argv);
			tryCount++;
		}
	}
	return ret;
...
int wpa_cli_main(int argc, char *argv[], wfdP2PCallback callback)
{
	...
	if (wfdP2PCB) wfdP2PCB(WFD_CB_QUITTED, NULL, 0);
