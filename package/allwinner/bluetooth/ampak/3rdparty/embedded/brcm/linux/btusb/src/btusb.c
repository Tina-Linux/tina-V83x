...
#if defined(BTUSB_LITE)
    while (GKI_dequeue(&p_dev->lite_cb.s.to_app.ipc_queue)!=NULL)
    {
        BTUSB_DBG("dequeued all remaining buffers from ipc_queue\n");
    }
#endif
    return 0;
}
...
int btusb_resume(struct usb_interface *p_interface)
{
    ...
    /* dequeue all data in rx queue */
    while (GKI_dequeue(&p_dev->rx_queue) != NULL)
    {
        BTUSB_INFO("GKI_dequeued !!\n");
    }
