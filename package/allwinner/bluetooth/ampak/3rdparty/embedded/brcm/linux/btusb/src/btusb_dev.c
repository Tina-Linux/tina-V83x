...
ssize_t btusb_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *ppos)
{
    ...
    while (remainder)
    {
        ...
        if (likely(p_dev->write_msg_len >= len))
        {
            ...
            if (autopm)
            {
                BTUSB_DBG("call usb_autopm_get_interface\n");
...
void btusb_urb_out_complete(struct urb *p_urb)
{
    ...
    if (autopm)
    {
        BTUSB_DBG("call usb_autopm_put_interface_async\n");
        usb_autopm_put_interface_async(p_dev->p_main_intf);
    }
...
/*******************************************************************************
 **
 ** Function         btusb_adjust_if_firmware_dl_packet
 **
 ** Description:     Adjust packet if this is firmware download packet
 **                  It has a special ACL connection handle value 0xBEEF which
 **                  was inserted by BTDL. A normal ACL packet will never have such
 **                  connection handle.
 **                  If this is detected, the handle and the following 2 bytes (length)
 **                  are skipped, since the WL firmware doing the download is not
 **                  expecting to see these 4 bytes.
 **
 ** Parameters       packet: HCI ACL packet to transmit
 **                  length: length of the HCI packet
 **
 ** Returns          1 if packet adjusted, 0 for no adjustment
 **
 *******************************************************************************/
int btusb_adjust_if_firmware_dl_packet(char* *packet, unsigned long *length)
{
...
int btusb_submit_acl(struct btusb *p_dev, char *packet, unsigned long length)
{
    ...
    if (btusb_adjust_if_firmware_dl_packet (&packet, &length))
