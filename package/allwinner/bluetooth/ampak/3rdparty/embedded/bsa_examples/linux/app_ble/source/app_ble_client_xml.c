...
int app_ble_client_xml_write(tAPP_BLE_CLIENT_DB_ELEMENT *p_element)
{
    ...
    while (element != NULL)
    {
           ...
           element->bd_addr[3]|element->bd_addr[4]|element->bd_addr[5])
        {
            ...
            APP_INFO1("save #2 BDA:%02X:%02X:%02X:%02X:%02X:%02X, client_if= %d ",
            ...
            while(attr != NULL)
            {
                APP_INFO1("attr save hanble:0x%x type:0x%x uuid:0x%04X, len:%d id:0x%x, prop:0x%x, primary:%d",
