...
static void *suspend_thread_func(void *arg __attribute__ ((unused)))
{
	...
	while (1) {
		...
		if (ret < 0) {
			strerror_r(errno, buf, sizeof(buf));
			TLOGE("Error waiting on semaphore: %s\n", buf);
			continue;
		}

		success = true;
		TLOGV("%s: write %*s to wakeup_count\n", __func__, wakeup_count_len, wakeup_count);
		...
		if (ret < 0) {
			strerror_r(errno, buf, sizeof(buf));
			TLOGE("Error releasing semaphore: %s\n", buf);
		}
	}
	return NULL;
}
...
static int autosuspend_wakeup_count_enable(void)
{
	...
	TLOGV("autosuspend_wakeup_count_enable\n");

	ret = sem_post(&suspend_lockout);

	if (ret < 0) {
		strerror_r(errno, buf, sizeof(buf));
		TLOGE("Error changing semaphore: %s\n", buf);
	}

	TLOGV("autosuspend_wakeup_count_enable done\n");
...
static int autosuspend_wakeup_count_disable(void)
{
	...
	TLOGV("autosuspend_wakeup_count_disable\n");
	...
	TLOGV("autosuspend_wakeup_count_disable done\n");
...
struct autosuspend_ops *autosuspend_wakeup_count_init(void)
{
	...
	if (ret < 0) {
		strerror_r(errno, buf, sizeof(buf));
		TLOGE("Error creating semaphore: %s\n", buf);
		goto err_sem_init;
	}
