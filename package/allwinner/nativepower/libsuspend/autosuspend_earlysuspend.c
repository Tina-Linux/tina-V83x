...
int wait_for_fb_wake(void)
{
	...
	if (err < 0)
		TLOGE("*** ANDROID_WAIT_FOR_FB_WAKE failed (%s)", strerror(errno));
	close(fd);
	return err < 0 ? err : 0;
}
...
static int wait_for_fb_sleep(void)
{
	...
	err = TEMP_FAILURE_RETRY(read(fd, &buf, 1));
	if (err < 0)
		TLOGE("*** ANDROID_WAIT_FOR_FB_SLEEP failed (%s)", strerror(errno));
	close(fd);
	return err < 0 ? err : 0;
}
...
static void *earlysuspend_thread_func(void __unused * arg)
{
	while (1) {
		if (wait_for_fb_sleep()) {
			TLOGE("Failed reading wait_for_fb_sleep, exiting earlysuspend thread\n");
			return NULL;
		}
		...
		if (wait_for_fb_wake()) {
			TLOGE("Failed reading wait_for_fb_wake, exiting earlysuspend thread\n");
			return NULL;
		}
		pthread_mutex_lock(&earlysuspend_mutex);
		earlysuspend_state = EARLYSUSPEND_ON;
...
static int autosuspend_earlysuspend_enable(void)
{
...
#if 1
	TLOGV("autosuspend_earlysuspend_enable\n");
	...
	TLOGV("autosuspend_earlysuspend_enable done\n");
#endif
...
static int autosuspend_earlysuspend_disable(void)
{
	...
	TLOGV("autosuspend_earlysuspend_disable\n");
	...
	TLOGV("autosuspend_earlysuspend_disable done\n");
...
void start_earlysuspend_thread(void)
{
	...
	TLOGI("Starting early suspend unblocker thread\n");
