...
static int AsfStreamingParseHeader(int fd, aw_mms_inf_t* mmsStreamInf)
{
    ...
    do
    {
        ...
        if(chunkSize2read==0)
        {
            ...
            chunkSize2read = CDX_RL64(&asfh->objh.size);  /* header object size*/
       }
    ...
    if(pos >= 0)
    {
        aw_asf_file_header_t *fileh = (aw_asf_file_header_t *) &buffer[pos];
        pos += sizeof(aw_asf_file_header_t);
        if (pos > size)
        {
            goto len_err_out;
        }
        asfCtrl->packetSize = AV_RL32(&fileh->maxPacketSize);
        mmsStreamInf->fileSize  = CDX_RL64(&fileh->fileSize);
        mmsStreamInf->fileDuration = (CDX_RL64(&fileh->playDuration)/10000);
        // before playing.
...
static int MmshStreamingRead( int fd, char *buffer, int size, aw_mms_inf_t* mmsStreamInf)
{
    ...
    if(chunk.type == ASF_STREAMING_HEADER)
    {
        ...
        if(pos >= 0)
        {
            fileh = (aw_asf_file_header_t *) &headerBuffptr[pos];
            asfHttpCtrl->packetSize = AV_RL32(&fileh->maxPacketSize);
