...
static int RtspRollOverTcp( CdxRtspStreamImplT *impl) //* todo
{
    ...
    for( i = 0; i < pSys->iTrack; i++ )
    {
        ...
        //if( tk->bMuxed ) stream_Delete( tk->pOutmuxed );
        //if( tk->pEs ) es_out_Del( impl->out, tk->pEs );
        //if( tk->pasfblock ) block_Release( tk->p_asf_block );
...
static void ReadAsfDataToBuffer(CdxRtspStreamImplT *impl, char bMarker, void *buf, cdx_uint32 len)
{
    cdx_int8 *data = (cdx_int8 *)buf;
    const cdx_uint32 pktSize = impl->pSys->asfh.minDataPktSize;
    cdx_int8 *tempBuf = NULL;
    cdx_uint32  iBuf = 0;
    ...
    while(len >= 4)
    {
        ...
        cdx_int8 bLength         = iFlags & 0x40;
        cdx_int8 bRelativeTs     = iFlags & 0x20;
        cdx_int8 bDuration       = iFlags & 0x10;
        cdx_int8 bLocationId     = iFlags & 0x08;
        ...
        if(!tempBuf)
        {
            tempBuf = (cdx_int8 *)malloc(pktSize);
            if(!tempBuf)
            {
                loge("malloc failed.");
                return ;
...
__exit:
        pthread_mutex_lock(&impl->lock);
        //CdxAtomicSet(&impl->state, RTSP_STREAM_IDLE);
        pthread_mutex_unlock(&impl->lock);
...
static int ExtractMediaInfo(MediaSubsession *sub, CdxRtspStreamImplT *impl)
{
    ...
    tk = (LiveTrackT *)malloc( sizeof( LiveTrackT ) );
    if( !tk )
    {
        loge("malloc failed.");
        return -1;
