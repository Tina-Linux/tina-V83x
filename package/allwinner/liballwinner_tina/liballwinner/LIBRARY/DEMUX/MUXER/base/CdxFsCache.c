...
typedef struct tag_FsCacheThreadContext
{
    ...
    volatile cdx_int8  *m_write_ptr;      //fwrite modify it, thread read it.
    volatile cdx_int8  *m_read_ptr;       //fwrite read it, thread modify it.
...
static ssize_t FsCacheThreadWrite(CdxFsWriter *thiz, const cdx_int8 *buf, cdx_int32 size)
{
    FsCacheThreadContext *p_ctx = (FsCacheThreadContext*)thiz->m_priv;
    cdx_int8 *p_rd_ptr;
    cdx_int8 *p_wr_ptr;
    cdx_int32 n_free_size;
...
static cdx_int32 FsCacheThreadSeek(CdxFsWriter *thiz, cdx_int64 noffset, cdx_int32 nwhere)
{
    ...
    cdx_int32 ret;
    thiz->fsFlush(thiz);
    ret = p_ctx->mp_writer->cdxSeek(p_ctx->mp_writer, noffset, nwhere);
    return ret;
}
...
static cdx_int64 FsCacheThreadTell(CdxFsWriter *thiz)
{
    ...
    thiz->fsFlush(thiz);
    n_file_pos = p_ctx->mp_writer->cdxTell(p_ctx->mp_writer);
    return n_file_pos;
}
...
{
    FsCacheThreadContext *p_ctx = (FsCacheThreadContext*)thiz->m_priv;
    cdx_int8 *p_rd_ptr = (cdx_int8*)p_ctx->m_read_ptr;
    cdx_int8 *p_wr_ptr = (cdx_int8*)p_ctx->m_write_ptr;
...
EXIT:
    logv("(f:%s, l:%d) FsCacheWriteThread[%d] will quit now.",
        __FUNCTION__, __LINE__, p_ctx->m_thread_id);
    return NULL;
}

CdxFsWriter *InitFsCacheThreadContext(CdxWriterT *p_writer, cdx_int8 *p_cache,
