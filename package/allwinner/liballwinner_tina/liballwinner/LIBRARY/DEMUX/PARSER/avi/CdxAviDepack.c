...
cdx_int64 ParseTime(cdx_uint8 *buf)
{
    ...
    cdx_int8 sec[3]    = {0};
    cdx_int8 msec[4]   = {0};
    cdx_int8 *tempBuf = (cdx_int8 *)buf;
    int h, min, s, ms;

    if (buf[2] != ':' || buf[5] != ':' || buf[8] != '.')
    {
        CDX_LOGE("in valid time format.");
        return -1;
    }

    strncpy(hour, tempBuf, 2);
    strncpy(minute, tempBuf+3, 2);
    strncpy(sec, tempBuf+6, 2);
    strncpy(msec, tempBuf+9, 3);

    h = strtol(hour, NULL, 10);
    min = strtol(minute, NULL, 10);
    s = strtol(sec, NULL, 10);
    ms = strtol(msec, NULL, 10);
...
cdx_int16 AVIReaderOpenFile(CdxAviParserImplT *impl)
{
    ...
    aviFile->avih->buffer =(cdx_int8 *)malloc((int)chunk->length);
            ...
            if(strmHdr->fccType == CDX_CKID_STREAM_TYPE_VIDEO) //bitmap form of subtitles.
            {
                switch(pStrmInfo->aviSubTag)
                {
                    ...
                    default:
                    {
                        ...
                        p->aFormatArray[i].eCodecFormat = SUBTITLE_CODEC_UNKNOWN;
                        break;
                    }
                }
            }
...
CdxAviParserImplT *AviInit(cdx_int32 *ret)
{
    ...
    *ret = 0;
    ...
    p->videoStreamIndex = (cdx_int8)-1;
    p->audioStreamIndex = (cdx_int8)-1;
