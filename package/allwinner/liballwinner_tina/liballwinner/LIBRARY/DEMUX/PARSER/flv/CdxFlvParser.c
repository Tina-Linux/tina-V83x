...
cdx_int16 FlvReaderOpenFile(CdxFlvParserImplT *impl)
{
    ...
    s->seekAble = CdxStreamSeekAble(impl->stream);
    ...
    //--------------------------------------------------------------------------
    if(s->seekAble == 1 && FlvBuildKeyFrameIndex(s) < 0)
    {
            ...
            if(impl->aFormat.eCodecFormat == AUDIO_CODEC_FORMAT_MPEG_AAC_LC)
            {
                ...
                if(impl->aFormat.nCodecSpecificDataLen)
                {
                    impl->aFormat.pCodecSpecificData=
                        (cdx_int8 *)malloc(impl->aFormat.nCodecSpecificDataLen);
        ...
        switch(mode)
        {
            ...
            default:
            {
                ...
                while(1)
                {
                    ...
                    if(result < 0)
                    {
                        CDX_LOGW("Seek back to the next tag header failed.");
                        impl->mErrno = PSR_UNKNOWN_ERR;
                        ret = -1;
                        goto __exit;
                    }
