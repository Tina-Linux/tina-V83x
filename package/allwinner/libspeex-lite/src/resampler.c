...
//        printf("%s,l:%d, framesWr:%d, outFrames:%d, inFrames:%d, rsmp->frames_in:%d\n", __func__, __LINE__, framesWr, outFrames, inFrames, rsmp->frames_in);
...
//         inSampleRate, outSampleRate, channelCount);
    ...
    *resampler = NULL;
    ...
    rsmp->in_buf = NULL;
    rsmp->in_buf_size = 0;
printf("%s,l:%d,in_rate:%d,out_rate:%d,ch:%d\n", __func__, __LINE__, inSampleRate, outSampleRate, channelCount);
