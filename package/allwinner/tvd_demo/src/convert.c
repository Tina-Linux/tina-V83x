...
int YUV422PToRGB24(void *RGB24,void *YUV422P,int width,int height)
{
    ...
    for(int y = 0;y < height;y ++)
    {
        for(int x = 0;x < width;x ++)
        {
            ...
            temp[0] = src_y[Y] + 1.772 * (src_u[U]-128);  //b
            temp[1] = src_y[Y] - 0.34414* (src_u[U]-128) - 0.71414 * (src_v[V]-128);  //g
            temp[2] = src_y[Y] + 1.402 * (src_v[V]-128);  //r
...
int YUVToRGB(const char *rgb_path,char *yuv_data,ConverFunc func,int width,int height)
{
