...
/*  if (!android::base::ReadFileToString(MAX_BRIGHTNESS_FILE, &content)) {
    std::cout << "Failed to read max brightness";
    return false;
  }*/
...
/*  if (!android::base::WriteStringToFile(std::to_string(brightness_normal_value_),
  ...
  std::cout << "Brightness: " << brightness_normal_value_ << " (" << brightness_normal_ << "%)";
...
bool RecoveryUI::Init(const std::string& locale) {
  ...
  if (touch_screen_allowed_) {
    ...
    if(false) {
      ...
      std::cout << "Failed to read /proc/cmdline";
    }
  }

  if (!InitScreensaver()) {
    std::cout << "Screensaver disabled";
  }
...
void RecoveryUI::OnTouchDetected(int dx, int dy) {
  ...
  if (abs(dy) < kTouchLowThreshold && abs(dx) > kTouchHighThreshold) {
    ...
    std::cout << "Ignored " << dx << " " << dy << " (low: " << kTouchLowThreshold
  ...
  std::cout << "Swipe direction=" << direction;
  switch (direction) {
...
/*          if (android::base::WriteStringToFile(std::to_string(brightness_dimmed_value_),
                                               BRIGHTNESS_FILE)) {*/
        	if(true) {
            std::cout << "Brightness: " << brightness_dimmed_value_ << " (" << brightness_dimmed_
                      << "%)";
