...
void ScreenRecoveryUI::LoadBitmap(const char* filename, GRSurface** surface) {
  ...
  if (result < 0) {
    std::cout << "couldn't load bitmap " << filename << " (error " << result << ")";
  }
}
...
void ScreenRecoveryUI::LoadAnimation() {
  std::unique_ptr<DIR, decltype(&closedir)> dir(opendir("/usr/share/system"), closedir);
  dirent* de;
