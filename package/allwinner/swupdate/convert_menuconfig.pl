...
while (<FIND>) {
	...
	while ($line = <INPUT>) {
		...
		if ($line =~ /^(\s*range\s*)(\w+)(\s+)(\w+)\s*$/) {
			...
			my $r1 = $2;
			my $r2 = $4;
			$r1 =~ s/^([a-zA-Z]+)/SWUPDATE_CONFIG_$1/;
			$r2 =~ s/^([a-zA-Z]+)/SWUPDATE_CONFIG_$1/;
			$line = "$prefix$r1 $r2\n";
		}

		$line =~ s/^(\s*(prompt "[^"]+" if|config|depends|depends on|select|default|default \w if)\s+\!?)([A-Z_])/$1SWUPDATE_CONFIG_$3/g;
		$line =~ s/(( \|\| | \&\& | \( )!?)([A-Z_])/$1SWUPDATE_CONFIG_$3/g;
		$line =~ s/(\( ?!?)([A-Z_]+ (\|\||&&))/$1SWUPDATE_CONFIG_$2/g;
