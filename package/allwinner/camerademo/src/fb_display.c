...
void setDisplay(int fh, struct fb_var_screeninfo *var);
...
static inline int read_lcd_info(char *file, char *buf, char buf_len)
{
	size_t len;
...
static int get_lcd_size(int *x, int *y)
{
#define LCD_X ("/sys/firmware/devicetree/base/soc@03000000/lcd0@01c0c000/lcd_x")
#define LCD_Y ("/sys/firmware/devicetree/base/soc@03000000/lcd0@01c0c000/lcd_y")
	char buf[32];

	*x = 240;
	*y = 240;

	if(read_lcd_info(LCD_X, buf, sizeof(buf)) == 4) {
		*y = (int)((buf[0]<<24) | (buf[1]<<16) | (buf[2]<<8) | (buf[3]));
	}

	if(read_lcd_info(LCD_Y, buf, sizeof(buf)) == 4) {
               ...
               unsigned int x_offs, unsigned int y_offs)
{
	...
	var.reserved[0] = 0; /* x */
	var.reserved[1] = 0; /* y */
	var.reserved[2] = lcd_x; /* w */
	var.reserved[3] = lcd_y; /* h */
