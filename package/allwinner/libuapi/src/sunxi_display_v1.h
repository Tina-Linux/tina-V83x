 ...
 *
 * Header fle for Display driver on sunxi platform
 *
...
typedef enum {
	...
	DISP_FORMAT_YUV444_P_TILE = 0x5C,
	DISP_FORMAT_YUV422_P_TILE = 0x5D,
	DISP_FORMAT_YUV420_P_TILE = 0x5E,
	DISP_FORMAT_YUV411_P_TILE = 0x5F,
} disp_pixel_format;
...
typedef enum {
	DISP_MOD_DE = 0,
	DISP_MOD_DEVICE = 1,	/* for timing controller common module */
	DISP_MOD_FE0 = 3,
	DISP_MOD_FE1 = 4,
	DISP_MOD_FE2 = 5,
	DISP_MOD_BE0 = 6,
...
/* disp_fb_info - image buffer info
 *
 * @addr: buffer address for each plane
 * @size: size<width,height>, unit:pixels
 * @format: pixel format
 * @cs_mode: color space
 ...
 * @src_win: crop rectangle for buffer to be display
 */
typedef struct {
	unsigned int addr[3];
	disp_size size;
	disp_pixel_format format;
	disp_cs_mode cs_mode;
	int b_trd_src;
	disp_3d_src_mode trd_mode;
 ...
 * @ck_enable: enable color key function
 ...
 *	DISP_CMD_LAYER_GET_FRAME_ID ioctl
 */
typedef struct {
...
typedef struct {
	...
	int (*hdmi_get_vendor_id)(unsigned char *id);
...
typedef struct {
	...
	/* color depth, 0:8bit; 1:6bit */
	disp_lcd_lvds_colordepth lcd_lvds_colordepth;
...
struct sunxi_disp_source_ops {
	...
	int (*sunxi_lcd_pwm_set_duty_ns)(unsigned int pwm_channel,
					  unsigned int duty_ns);
	int (*sunxi_lcd_pwm_enable)(unsigned int pwm_channel);
	int (*sunxi_lcd_pwm_disable)(unsigned int pwm_channel);
	int (*sunxi_lcd_cpu_write)(unsigned int scree_id, unsigned int command,
				    unsigned int *para, unsigned int para_num);
	int (*sunxi_lcd_cpu_write_index)(unsigned int scree_id,
					  unsigned int index);
	int (*sunxi_lcd_cpu_write_data)(unsigned int scree_id,
					 unsigned int data);
	int (*sunxi_lcd_dsi_write)(unsigned int scree_id,
				    unsigned char command, unsigned char *para,
	...
	int (*sunxi_lcd_gpio_set_value)(unsigned int screen_id,
					 unsigned int io_index, unsigned int value);
...
typedef enum {
	...
	FB_MODE_SCREEN1 = 1,
	FB_MODE_SCREEN2 = 2,
	FB_MODE_DUAL_SAME_SCREEN_TB = 3,
	/* two screen, they have same contents; */
	FB_MODE_DUAL_DIFF_SCREEN_SAME_CONTENTS = 4,
} disp_fb_mode;
...
typedef struct {
	...
	unsigned int primary_screen_id;
	unsigned int aux_output_width;
	unsigned int aux_output_height;
} disp_fb_create_info;
...
typedef enum {
	...
	DISP_INIT_MODE_TWO_DIFF_SCREEN = 3,
	/* fb0(up buffer for screen0, down buffer for screen1) */
	DISP_INIT_MODE_TWO_SAME_SCREEN = 4,
	/*
...
typedef enum tag_DISP_CMD {
	...
	/* ---- capture --- */
	DISP_CMD_CAPTURE_SCREEN = 0x140,
	DISP_CMD_CAPTURE_SCREEN_STOP = 0x141,
