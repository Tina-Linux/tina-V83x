...
upgrade_pre_mode(){
    ...
    fgrep -sq " /usr " /proc/mounts && {
	    # umount the usr partition; if failed, ignore
	    umount /usr # unmount overlayfs
	    umount /usr # unmount /dev/by-name/extend
    }
    ...
    if [ x$IS_COMPRESS_IMAGE = x"--none-compress" ] \
	    && [ -f $LOCAL_IMG_PATH/$RAMDISK_IMG ] \
	    && [ -f $LOCAL_IMG_PATH/$BOOT_IMG ] \
	    && [ -f $LOCAL_IMG_PATH/$ROOTFS_IMG ] \
	    && [ -f $LOCAL_IMG_PATH/$USR_IMG ]; then
            $UPGRADE_SH prepare $LOCAL_IMG_PATH $RAMDISK_IMG $IS_COMPRESS_IMAGE
            [ x"$REBOOT_TO_RECOVERY" = x"1" ] || {
            #don't need to prepare others now, as we will reboot after upgrade ramdisk
            $UPGRADE_SH prepare $LOCAL_IMG_PATH $BOOT_IMG $IS_COMPRESS_IMAGE
            $UPGRADE_SH prepare $LOCAL_IMG_PATH $ROOTFS_IMG $IS_COMPRESS_IMAGE
            }
            $UPGRADE_SH upgrade
    elif [ -f $LOCAL_IMG_PATH/$RAMDISK_PKG ] \
	    && [ -f $LOCAL_IMG_PATH/$TARGET_PKG ] \
	    && [ -f $LOCAL_IMG_PATH/$USR_PKG ]; then
            ...
            [ -f $LOCAL_IMG_PATH/$BOOT0_PKG ] && \
                $UPGRADE_SH prepare $LOCAL_IMG_PATH $BOOT0_PKG $IS_COMPRESS_IMAGE
            [ -f $LOCAL_IMG_PATH/$UBOOT_PKG ] && \
                $UPGRADE_SH prepare $LOCAL_IMG_PATH $UBOOT_PKG $IS_COMPRESS_IMAGE
            }
            $UPGRADE_SH upgrade
    else
...
upgrade_target_mode(){
    ...
    if [ x$IS_COMPRESS_IMAGE = x"--none-compress" ] \
        ...
        && [ -f $LOCAL_IMG_PATH/$USR_IMG ] ; then
            $UPGRADE_SH prepare $LOCAL_IMG_PATH $BOOT_IMG $IS_COMPRESS_IMAGE
            $UPGRADE_SH prepare $LOCAL_IMG_PATH $ROOTFS_IMG $IS_COMPRESS_IMAGE
            [ -f $LOCAL_IMG_PATH/$BOOT0_IMG ] && \
            $UPGRADE_SH prepare $LOCAL_IMG_PATH $BOOT0_IMG $IS_COMPRESS_IMAGE
            [ -f $LOCAL_IMG_PATH/$UBOOT_IMG ] && \
            $UPGRADE_SH prepare $LOCAL_IMG_PATH $UBOOT_IMG $IS_COMPRESS_IMAGE
            $UPGRADE_SH prepare $LOCAL_IMG_PATH $USR_IMG $IS_COMPRESS_IMAGE
            $UPGRADE_SH upgrade
    elif [ -f $LOCAL_IMG_PATH/$TARGET_PKG ] && [ -f $LOCAL_IMG_PATH/$USR_PKG ]; then
        $UPGRADE_SH prepare $LOCAL_IMG_PATH $TARGET_PKG $IS_COMPRESS_IMAGE
        $UPGRADE_SH upgrade
        $UPGRADE_SH prepare $LOCAL_IMG_PATH $USR_PKG $IS_COMPRESS_IMAGE
        $UPGRADE_SH upgrade
...
upgrade_post_mode(){
    ...
    if [ x$IS_COMPRESS_IMAGE = x"--none-compress" ] && [ -f $LOCAL_IMG_PATH/$USR_IMG ]; then
	$UPGRADE_SH prepare $LOCAL_IMG_PATH $USR_IMG $IS_COMPRESS_IMAGE
    elif [ -f $LOCAL_IMG_PATH/$USR_PKG ]; then
        $UPGRADE_SH prepare $LOCAL_IMG_PATH $USR_PKG $IS_COMPRESS_IMAGE
    else
        check_network $DOMAIN
        download_prepare_image $USR_PKG $IS_COMPRESS_IMAGE
...
while getopts "fpl:nu:d:" opt; do
    ...
    f)
        check_mode
        mode="--force"
        [ x"$STREAM_UPDATE" = x"1" ] && set_system_flag "upgrade_pre"
        ;;
    p)
        check_mode
        mode="--post"
...
if [ x$system_flag = x"upgrade_pre" ]; then
    ...
    upgrade_target_mode
elif [ x$system_flag = x"upgrade_boot0" ]; then
    upgrade_target_mode
elif [ x$system_flag = x"upgrade_uboot" ]; then
    upgrade_target_mode
