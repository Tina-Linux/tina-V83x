...
check_network(){
    ...
    [ $? -ne 0 ] && {
        ...
        local flag=$(get_system_flag)
        echo "### $flag ###"
        [ x$flag = x"boot-recovery" ] && {
            write_misc -c upgrade_end
            sync
            reboot -f
        }
        exit $ERR_VENDOR_HOOK_NOT_SUPPORT
...
upgrade_pre_mode(){
    ...
    # ------>write image "boot","rootfs" to "extend" partition
	...
	&& [ -f $LOCAL_IMG_PATH/$ROOTFS_IMG ]; then
	...
	$UPGRADE_SH upgrade
	    ...
	    && [ -f $LOCAL_IMG_PATH/$TARGET_PKG ]; then
...
upgrade_target_mode(){
	...
	&& [ -f $LOCAL_IMG_PATH/$ROOTFS_IMG ]; then
        ...
        $UPGRADE_SH prepare $LOCAL_IMG_PATH $ROOTFS_IMG $IS_COMPRESS_IMAGE
        $UPGRADE_SH upgrade
    elif [ -f $LOCAL_IMG_PATH/$TARGET_PKG ]; then
	$UPGRADE_SH prepare $LOCAL_IMG_PATH $TARGET_PKG $IS_COMPRESS_IMAGE
	[ -f $LOCAL_IMG_PATH/$BOOT0_PKG ] && \
	$UPGRADE_SH prepare $LOCAL_IMG_PATH $BOOT0_PKG $IS_COMPRESS_IMAGE
	[ -f $LOCAL_IMG_PATH/$UBOOT_PKG ] && \
