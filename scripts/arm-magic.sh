...
do
  ...
  if [ "$2" -lt "256" ]; then
    ...
    printf "\xe3\xa0\x10\x$hexid" > $TARGET_OUT_DIR/$IMG_PREFIX-$1-zImage
  else
    # we have a high machtypeid, we need a "mov" (e3a) and an "orr" (e38)
    if [ "$2" -lt "4096" ]; then
      printf "\xe3\xa0\x10\x$(echo $hexid|cut -b "2 3")\xe3\x81\x1c\x$(echo $hexid|cut -b 1)" > $TARGET_OUT_DIR/$IMG_PREFIX-$1-zImage
    else
      printf "\xe3\xa0\x10\x$(echo $hexid|cut -b "3 4")\xe3\x81\x1c\x$(echo $hexid|cut -b "1 2")" > $TARGET_OUT_DIR/$IMG_PREFIX-$1-zImage
    fi
  fi
    # generate the image
    cat $TARGET_OUT_DIR/$IMG_PREFIX-zImage >> $TARGET_OUT_DIR/$IMG_PREFIX-$1-zImage
done
