...
KERNEL_CROSS:=$(ARCH)-openwrt-linux-
...
#
# $(1); the input file for initramfs
#
...
ifneq ($(CONFIG_KERNEL_CONFIG_FILE_SUFFIX_RECOVERY),)
	$(LINUX_CONF_CMD_RECOVERY) > $(LINUX_DIR)/.config.target
else ifneq ($(CONFIG_KERNEL_CONFIG_FILE_SUFFIX_RAMFS),)
	$(LINUX_CONF_CMD_RAMFS) > $(LINUX_DIR)/.config.target
else
...
ifeq ($(CONFIG_COMPLILE_KERNEL64_USER32),y)
	$(_SINGLE) [ -d $(LINUX_DIR)/user_headers ] || $(MAKE) $(subst arm64,arm,$(KERNEL_MAKEOPTS)) \
...
define Kernel/CopyImage
	cmp -s $(LINUX_DIR)/vmlinux $(KERNEL_BUILD_DIR)/vmlinux$(1).debug || { \
		...
		$(CP) $(LINUX_DIR)/arch/$(LINUX_KARCH)/boot/$(IMAGES_DIR)/Image $(KERNEL_BUILD_DIR)/bImage$(1); \
		$(foreach k, \
...
define Kernel/CompileImage/Initramfs
	$(foreach hook,$(Hooks/mkfs/Pre),$(call $(hook),initramfs,root)$(sep))
	$(call Kernel/Configure/Initramfs,$(if $(Image/Input),$(Image/Input),$(TARGET_DIR)))
