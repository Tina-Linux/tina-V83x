...
ifndef ($(CONFIG_USE_SELINUX),)
  FILE_CONTEXTS := $(TOPDIR)/package/security/sepolicy-demo/src/file_contexts
endif
...
ifneq ($(CONFIG_USE_SELINUX),)
	$(STAGING_DIR_HOST)/bin/mksquashfs4_se $(1) $(KDIR)/$(2) -noappend -root-owned -comp $(SQUASHFS_COMP) $(SQUASHFSOPT) -processors $(if $(CONFIG_PKG_BUILD_JOBS),$(CONFIG_PKG_BUILD_JOBS),1) $(if $(SOURCE_DATE_EPOCH),-fixed-time $(SOURCE_DATE_EPOCH)) -context-file $(FILE_CONTEXTS) -mount-point /
else
	$(STAGING_DIR_HOST)/bin/mksquashfs4 $(1) $(KDIR)/$(2) -noappend -root-owned -comp $(SQUASHFS_COMP) $(SQUASHFSOPT) -processors $(if $(CONFIG_PKG_BUILD_JOBS),$(CONFIG_PKG_BUILD_JOBS),1) $(if $(SOURCE_DATE_EPOCH),-fixed-time $(SOURCE_DATE_EPOCH))
endif
ifneq ($(CONFIG_SUNXI_SMALL_STORAGE_OTA),)
