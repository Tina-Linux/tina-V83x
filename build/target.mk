...
ifeq ($(DUMP),)
  ...
  DEVICE_PLATFORM_DIR:=$(TOPDIR)/device/config/chips/$(shell echo -n $(BOARD) | sed -e 's/-.*//g')/configs/$(shell echo -n $(BOARD) | sed -e 's/[^-]*-//g')/linux
...
__config_name_list = $(1)/config-$(KERNEL_PATCHVER) $(1)/config-default $(2)/config-$(KERNEL_PATCHVER) $(2)/config-default
...
__config_name_list_recovery = $(1)/config-$(KERNEL_PATCHVER)_recovery $(1)/config-default_recovery $(2)/config-$(KERNEL_PATCHVER)_recovery $(2)/config-default_recovery
__config_list_recovery = $(firstword $(wildcard $(call __config_name_list_recovery,$(1),$(2))))
find_kernel_config_recovery=$(if $(__config_list_recovery),$(__config_list_recovery),$(lastword $(__config_name_list_recovery)))

__config_name_list_ramfs = $(1)/config-$(KERNEL_PATCHVER)_ramfs $(1)/config-default_ramfs $(2)/config-$(KERNEL_PATCHVER)_ramfs $(2)/config-default_ramfs
__config_list_ramfs = $(firstword $(wildcard $(call __config_name_list_ramfs,$(1),$(2))))
...
LINUX_TARGET_CONFIG = $(call find_kernel_config,$(DEVICE_PLATFORM_DIR),$(PLATFORM_DIR))
ifneq ($(PLATFORM_DIR),$(PLATFORM_SUBDIR))
...
LINUX_TARGET_CONFIG_RECOVERY = $(call find_kernel_config_recovery,$(DEVICE_PLATFORM_DIR),$(PLATFORM_DIR))
ifneq ($(PLATFORM_DIR),$(PLATFORM_SUBDIR))
...
LINUX_TARGET_CONFIG_RAMFS = $(call find_kernel_config_ramfs,$(DEVICE_PLATFORM_DIR),$(PLATFORM_DIR))
ifneq ($(PLATFORM_DIR),$(PLATFORM_SUBDIR))
